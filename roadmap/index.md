...
- **final** — ✓ STEP 1 DONE ([N+39](./N+39-property-spelling-registry.md)): the **property-spelling registry** ([decision 73](../decisionlog.md) cl. 2 · canonical id → `{ rn, css }`) extracted + `resolve-map.ts` homed into `@nuri/spec` **RN-free** (the [decision 68](../decisionlog.md) `rn → spec` DAG · the bespoke three `palette-surface`/`interactive-effects`/`typography-axis` were already in `spec`, so only `resolve-map.ts` needed relocating). REMAINING: the registry's adoption by the palette/interactive/typography emits (the spelling single-sourcing follow) · re-source `build/palette.ts` from `palette-surface.ts` · then the package carve.
