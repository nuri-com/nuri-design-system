...
### Repo structure · design-SoT ⊥ plumbing separation (surfaced N+27)

The web layer interleaves two things the operator wants physically separated
(2026-06-22 · the family of [decision 35](../decisionlog.md#35-pipeline-sources-vs-build-outputs-physically-separated--pipeline-source-build-generated-only--n604)
— pipeline-source vs build-output):

- **Design SoT** — the design decisions + pure data: tokens
  ([`styles/tokens-*.css`](../packages/spec/styles)), the `@layer` namespace
  mappings (which token a variant resolves to · the geometry scales · in
  `lib/components/*/*.css`), the descriptors. This is the **§9 generation
  target** (the build-time `descriptor → CSS` resolver EMITS the namespace CSS
  from `table × tokens`).
- **Plumbing** — hand-written web-component-environment boilerplate that
  resolves no token and encodes no design decision: the `.js` custom-element
  mechanism, the `display:contents` wrappers + `:not(:defined)` pre-upgrade
  skeletons, the flexbox `min-width:0`, the global `box-sizing` (shell.css),
  and the native-host **resets**.

Today they're mixed — every `lib/components/<name>/<name>.css` carries both its
`@layer rules` design-mappings AND its wrapper/skeleton plumbing; the `.js` is
pure mechanism. That mix is what makes the namespace CSS unsafe to generate
wholesale at §9: a generator emitting `interactive.css` would clobber any
hand-authored boilerplate folded into it (the N+27 reset veto · the operator
caught exactly this).

**Seeded at N+27**: [`lib/runtime/`](../packages/spec/lib/runtime) — the runtime
web mechanism (`factory.js` + `reset.css`, the first extracted plumbing). The
full pass collects the scattered boilerplate under that roof, settles the
per-component `.css` design/plumbing split, and **logs the decision** (a
structural choice in the family of decision 35). **Orthogonal to the
factory-rewrite S1–S4–§9 sequence** (pick up independently), but it MUST precede
or accompany **§9** — the CSS generator needs a clean "generate this / leave
that" boundary, and the [`reset.css`](../packages/spec/lib/runtime/reset.css)
header is today's boundary marker. See [`roadmap/N+27.md`](./N+27.md) ·
[`roadmap/factory-rewrite.md`](./factory-rewrite.md) §1.
