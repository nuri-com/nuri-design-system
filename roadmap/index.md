...
The token-cascade VOCABULARIES are TS-authored (descriptors B1 · dimensions [§71](../decisionlog.md) · colour [§72](../decisionlog.md) · the **type composite** [§78](../decisionlog.md) · [N+52](./N+52.md)); the residual hand-CSS primitives — the font-size/weight families · `--nuri-border-*` · font families · durations — stay a **Phase 4·3 residue** (decision 2 still stands for them · the inversion is NOT yet complete). The **L3 namespace flip** is DONE (L3c flipped it live · [N+38](./N+38-L3c-flip.md) · [decision 74](../decisionlog.md)) — box/stack ([N+30](./N+30-L3.1.md) shadow) + palette ([N+33](./N+33-L3b-palette.md) shadow) + interactive ([N+34](./N+34-L3b-interactive.md) shadow) + typography ([N+35](./N+35-L31b-typography.md) shadow) proven — **all 5 axis shadows are now DONE**. The axis taxonomy is **2 agnostic (`stack` · `box`) + 3 bespoke-SHAPE (`typography` · `palette` · `interactive`)** ([decision 73](../decisionlog.md) · corrects decision 70's "3 agnostic"); [decision 76](../decisionlog.md) (N+44) refines "bespoke" to mean SHAPE only — after interactive's single-sourcing **all 5 axes are single-sourced** (the one-SoT-two-projections invariant is now universal · bespoke ≠ "two sources").
...
- **final** — ✓ STEP 1 DONE ([N+39](./N+39-property-spelling-registry.md)): the **property-spelling registry** ([decision 73](../decisionlog.md) cl. 2 · canonical id → `{ rn, css }`) extracted + `resolve-map.ts` homed into `@nuri/spec` **RN-free** (the [decision 68](../decisionlog.md) `rn → spec` DAG · the bespoke three `palette-surface`/`interactive-effects`/`typography-axis` were already in `spec`, so only `resolve-map.ts` needed relocating). ✓ STEP 2 DONE ([N+40](./N+40.md)): **re-source `build/palette.ts`** from the TS SoTs (`palette-surface.ts` + `typography-axis.ts`) — `derivePalette` (Slice 8) now asserts the contract against the SoTs the namespace CSS is generated FROM, not the generated CSS itself, so `spec`'s build stops reading the namespace CSS the A3 carve moves out (data byte-identical · the named A3 blocker). REMAINING: the registry's adoption by the palette/interactive/typography emits (the spelling single-sourcing follow) · then the **package carve** (A3 / N+41 · **✓ SHIPPED** — `@nuri/prototype` carved · A4–A6 remain).

**The concrete remaining plan ([decision 80](../decisionlog.md) · [`docs/projection-model.md`](../docs/projection-model.md) · N+54):** the architecture after the inversion is now locked as a doc — **`@nuri/spec` is PURE DATA** (`tokens`/`axes`/`components`/`icons`), every consumer a **projection** that owns its resolution (web CSS → `prototype` · RN resolved-semantic hex → `rn`). **Phase 4·4 = `spec` data-only** per the slice plan ([§8](../docs/projection-model.md): accent reshape → add accents → kill `build/`/the round-trip/the descriptor `.ts` copies → catalog); the slice plan supersedes the loose "ledger purge" framing of Phase 5. See [`convergence.md`](./convergence.md) Phase 4·4 / 5.

**After the inversion (parked · separate arcs):** the **package carve** ([decision 68](../decisionlog.md)'s 6 packages · now near-mechanical — L3c leaves coherent folders · **A3 ✓ `@nuri/prototype` carved at [N+41](./N+41-prototype-carve.md)** · **A4 ✓ slice 1 `@nuri/doc` carved at [N+42](./N+42.md)** [decision 75] · **A4b ✓ the axis doc family at [N+43](./N+43.md)** · the **axis-doc redesign** STARTED at **[N+46](./N+46.md)** (the typography page re-pointed to the de-fused `size`+`emphasis` axis · the agnostic `| Input | Web | RN | Value |` grammar **LOCKED**) and the **fan-out ✓ LANDED at [N+47](./N+47.md)** (box/stack `CSS`→`Web` · interactive re-sourced onto `opts`/`webChrome`/`webOrder` + the demoted web-only `## Chrome` · the N+44 derived `effects` bridge DELETED · palette left byte-identical · the "CSS guide" critique CLOSED · [decision 76](../decisionlog.md)) now precedes **A4c ✓ the Foundations token family at [N+48](./N+48.md)** (colour/dimension/typography · §75's 3-family set CLOSED · the doc arc COMPLETE) · **A5 ✓ `@nuri/playground` carved at [N+49](./N+49.md)** (the build-free bench · the 6th & last package · the 6-package structure now REAL) · A6 codegen remains) · the remaining **token residues** (the type composite ✓ [§78](../decisionlog.md) [N+52](./N+52.md) · the font-size/weight primitives · `--nuri-border-*` · font families · durations → TS · Phase 4·3) · the **descriptor arc** (descriptors for the other ~16 hand-composed components → their recipe CSS retires · "everything is a composition of axes") · the **fidelity first-bump** (R1.5 defaults · stringly-boolean axes · `subtle` · topbar title-type).

## Current state

N+63 · **Phase 5 — the purge + the concise front door** — the infra refactor being complete (N+62 · `@nuri/spec` is data-only), this session makes the *docs* match: the coordinator-authored concise project doc is installed as the root **`README.md`** (replacing the 18KB structure-mirroring stale one · its README token-count drift pin dropped from `docs-drift.test.js` Guard C · the `llms.txt` pins intact), the sprawling historical record (**`decisionlog.md`** + **`roadmap/index.md`**) is **archived in place** (a banner header only · NO link surgery · the thousands of internal cross-links stay), the superseded model docs (`resolver-model` · `emit-model` · `composition-model` · `cascade` · `north-star` · `target-architecture` · `package-architecture` · **`projection-model`** — their *why* lives in the archived ledger) + the dead weight (`uploads/` · `closeout-audit.md` · the top-level `playground/` README · `docs/variants-model-spike.md`) are trashed (**`docs/` now holds only `RISKS.md` + `consumer-feedback/`** [the frozen R-EXPO evidence archive · kept as a live reference]), **`AGENTS.md`** is rewritten for the data-only structure (points at `README.md` · no operational decisionlog/roadmap refs) and the 7 dead `skills/` (they describe the deleted `lib/components`/`pages/`/`pipeline/` structure) are deleted (the `add-component` catalog procedure is authored at Phase 6 start). **No code/behaviour change · no SoT/codegen/generated-output edit** — the only gate that moves is `docs-drift` (the README pin intentionally dropped). **NEXT = Phase 6** (the catalog · the actual product · descriptors for the components still in `@nuri/prototype/legacy/`). See [`decisionlog.md`](../decisionlog.md) §80 (the *why* · archived).

N+62 · **THE INFRA EXIT — Slice 3 closes: `@nuri/spec` is data-only** — the single closing session for the projection-model refactor ([decision 80](../decisionlog.md) · [`docs/projection-model.md`](../docs/projection-model.md)), collapsing **3b·2b·ii + 3c + 3d into ONE mechanical session** (relocation · rename · repoint · **ZERO value change** · the 5 CI gates + the equivalence harnesses are the completeness oracle). Three movements: **(A)** the generated RN contract (`tokens` · `token-paths` · `interaction` · `icons` · `palette`) → **`@nuri/rn/generated/`** (committed · the codegen writes it · [`contract.ts`](../packages/rn/contract.ts) imports `./generated/*` · `@nuri/doc` reads `tokens`/`palette` from rn via the new [`strip.js`](../packages/doc/pipeline/strip.js)`#loadDataFromPath`) · **(B)** the web output (the token CSS · the 3 descriptor `.js` twins · `token-vars` · the web icon registry · `tokens.json`) → **`@nuri/prototype/generated/`** (the recipes · `primitives/icon.js` · playground HTML · `css-preview.js` · `stage.mjs` · the 4 prototype namespace tests · doc all repointed) · **(C)** rename `pipeline/` → **`tokens/`** (dimensions·colours·typography) + **`axes/`** (resolve-map·palette-surface·interactive-effects·typography-axis·property-spelling·interaction) + `descriptors/` → **`components/`** (the descriptors + schema) · **`build/`** + **`styles/`** + **`lib/`** DELETED · the 2 `*-computed-check.html` + the `scope/` tombstone gone · `icons/` (the SVG SoT) kept. The spec **exports map** drops the 6 generated subpaths (`tokens`/`token-paths`/`token-vars`/`interaction`/`icons`/`palette`) + repoints the descriptor subpaths → `components/` + the axis/SoT subpaths → `axes/`/`tokens/` (the subpath NAMES stay stable · only `build/`/`pipeline/` targets moved). The **freshness gate** ([`gates.yml`](../.github/workflows/gates.yml)) extends its diff to `packages/rn/generated packages/prototype/generated` (spec has nothing generated to diff). **`@nuri/spec` is now PURE DATA — tree = `tokens/ axes/ components/ icons/` + package.json — the projection-model end-state.** **Judgment call (noted · not a blocker):** `tokens.json` (DTCG · build-only · NO package consumer · only the round-trip drift guard reads it) relocated WITH the web output to `prototype/generated/` rather than dropped — keeps the guard + `buildDtcg`/`readPrimitives` alive while spec goes data-only. (on branch `feat/n62-infra-exit` · base `main` @ `3f2e445` [after [#89](https://github.com/nuri-com/nuri-design-system/pull/89) · N+61 merged] · [decision 80](../decisionlog.md) · **no new decision** [realizes §80's §8 slice plan] · **no value change**). **Gates green LOCALLY (all 5 + harnesses):** spec drift **60/60** + freshness clean (rn+proto generated byte-identical) · prototype **31** + styles byte-identical · doc **3** + generated re-emit (the ONLY md delta = the provenance `source:` headers re-pathed to the new homes) · **rn render-smoke 29 + 7 snapshots BYTE-IDENTICAL** + tsc 0 · expo tsc 0 · **all 15 relocated artifacts byte-identical to the pre-move baseline (sha-256 verified).** **Mutation reproduced:** `tokens/dimensions.ts` `px 48→49` → `rn/generated/tokens.ts` moved + the freshness guard + `dimension-tokens` drift bit · reverted clean. **THE INFRA ARC IS COMPLETE — EXIT to Phase 5** (the ledger/doc purge — fold `resolver-model`/`emit-model`/`composition-model` into [§64/65/70](../decisionlog.md)) **then Phase 6** (the catalog · the actual product · the NEXT thing). See [`roadmap/N+62.md`](./N+62.md) · [`decisionlog.md` §80 / §35](../decisionlog.md) · [`docs/projection-model.md`](../docs/projection-model.md).

N+61 · **Slice 3b·2b·i — relocation prep: interaction SoT + type-re-home (in place)** — the prep step for **3b·2b** (the irreversible RN-contract relocation): land the two preconditions the contract move needs, both **in place · byte-identical**, NO relocation. (1) **interaction micro-SoT** — `{ pressScale: 0.97, disabledOpacity: 0.4 }` had NO TS home (CSS-only · the N+60 STOP): new **[`pipeline/interaction.ts`](../packages/spec/pipeline/interaction.ts)** is now the single source; the build **generates BOTH readers from it** (the ratified **generate-from-SoT** path, not the drift-guard fallback — the 2-line CSS block was not web-entangled): **[`parsers/interaction.js`](../scripts/parsers/interaction.js)**`#flipInteractionCss` writes the `--nuri-interaction-*` CSS primitives in place (the type-css.js posture · bidirectional family drift guard) + `build/interaction.ts` flattens STRAIGHT from the SoT (`buildInteraction(interactionSoT)` · no CSS read) — the family's last TS→CSS→TS round-trip is DEAD. (2) **type-re-home** (the real fix for the 3a schema seam) — **[`pipeline/descriptors/schema.ts`](../packages/spec/pipeline/descriptors/schema.ts)** derives `SizeLeaf`/`Accent`/`TypeSize` from the TS SoTs (`keyof typeof import('../dimensions'|'../colours'|'../typography')`), so spec imports **NOTHING from `build/`** — seam (i) (the leaky `schema.ts → ../../build/tokens`) is CLOSED, the precondition for the contract leaving spec without a cycle (`@nuri/rn` depends on `@nuri/spec`). The derived unions are identical (rn tsc 0 confirms). (3) **orphan emit killed** — `build/descriptors/schema.ts` was an orphan since 3a (rn imports the SOURCE via the `./descriptors/schema` subpath): `git rm`'d + `emitSchemaTs`/`SCHEMA_HEADER` removed from [`descriptors.js`](../scripts/parsers/descriptors.js) + `SCHEMA_SRC`/the schema emit + log dropped from [`tokens-parser.js`](../scripts/tokens-parser.js) + the `.ts` re-emit assertion dropped from [`docs-drift.test.js`](../scripts/docs-drift.test.js) (**Guard F still freezes the SOURCE shape** · its `leafForms` pins re-homed onto the SoTs + `Accent`/`TypeSize` added). [`tokens-parser.test.js`](../scripts/tokens-parser.test.js) — the interaction guard rewritten **round-trip-dead** (on-disk ≡ emit-from-SoT [non-tautological] + the CSS flip idempotent at the SoT values). (on branch `feat/n61-rn-contract-prep` · base `main` @ `5461ff7` [after [#88](https://github.com/nuri-com/nuri-design-system/pull/88) · N+60 merged] · [decision 80](../decisionlog.md) · **no new decision** [the interaction SoT is data authoring · the type-re-home is operator-ratified under §80] · **no value change** — `build/*`+`styles/*` BYTE-IDENTICAL vs `main` save the `interaction.ts` provenance header + the orphan `git rm`). **Gates green (coordinator-verified):** spec drift **60/60** + `build/`+`styles/` byte-identical · prototype **31** + styles byte-identical · doc **3** + generated byte-identical · **rn 29 + 7 snapshots byte-identical** + tsc 0 · expo tsc 0. **Mutation reproduced (coordinator-verified):** `interaction.ts` `pressScale` 0.97→0.95 → BOTH `build/interaction.ts` AND the `--nuri-interaction-*` CSS moved (single-source · both readers track the SoT) + the drift guard bit · reverted clean. **Next — Slice 3 NO LONGER SUB-DIVIDES (PLAN · IMPORTANT):** with `interaction.ts` + the type-re-home done — the **two preconditions 3b·2b inherited** (seam (i) closed; the interaction SoT exists) — the residue of Slice 3 COLLAPSES into **ONE closing session**: **3b·2b·ii + 3c + 3d → a single EXIT-to-infra session** — relocate the RN contract (`build/tokens.ts` + the resolved-semantic) → `@nuri/rn` · the web projection (`styles/` · `token-vars` · the `.js` twins) → `@nuri/prototype` · doc→rn · **kill `build/`** · rename `pipeline/`→`tokens`/`axes`/`components`/ · extend the freshness gate to rn's (+ prototype's) generated dir (seam (ii) · the N+53 build-order) · A/B = **committed**. **One PR, one review, EXIT to the projection-model end-state.** See [`roadmap/N+61.md`](./N+61.md) · [`decisionlog.md` §80 / §35](../decisionlog.md) · [`docs/projection-model.md` §4](../docs/projection-model.md).

N+60 · **Slice 3b·2a — finish TS-sourcing the RN contract (in place)** — the second sub-slice of **Slice 3b**: kill the RN contract's last CSS round-trips so `build/tokens.ts` + `build/token-paths.ts` are generated with **zero CSS dependency in their value arm**, staying **in place** in `spec/build/` (the relocation to `@nuri/rn` is 3b·2b) (on branch `feat/n60-rn-contract-ts-source` · base `main` @ `891aea2` [after [#87](https://github.com/nuri-com/nuri-design-system/pull/87) · N+59 merged] · [decision 80](../decisionlog.md) · [`docs/projection-model.md` §4–§6](../docs/projection-model.md) · **no new decision** · **no value change** — `build/*`+`styles/*` BYTE-IDENTICAL vs `main` save the provenance-header lines on the two re-sourced files). N+59 (3b·1) re-sourced the COLOUR arm; this finishes the DIMENSION arm + the TokenPath union. **THE DESIGN CALL (ratified): the round-trip is REMOVED, not masked.** Rather than the literal 3b·1 Object.assign-over (walk the CSS, then overwrite), `resolveSemanticCrossProduct` is **dropped from the RN value path** — `resolved` is assembled `{ ...colourTokens.chrome, ...dimensionTokens }` from the two TS resolvers, so the dimension arm genuinely never reads `semanticRules` (Guard D is honest, not cosmetic). `classifyAll(semanticRules)` stays ONLY for the GROUP STRUCTURE + the web `token-vars` (relocates with the web projection · 3c). (1) **[`dimension-tokens.js`](../scripts/parsers/dimension-tokens.js)** (new) — `resolveDimensionTokens(dims)`: `space`/`size`/`radius` flattened ref→px straight from `pipeline/dimensions.ts` (`{ref:N}`→`${px[N]}px` · the `0`/`9999` sentinels verbatim · decision 32 / 36.1), returned in the cross-product shape so the generic singleton emit collapses it byte-identical (mirrors `colour-tokens.js`). (2) **[`token-paths.js`](../scripts/parsers/token-paths.js)** (new) — `emitTokenPathsTsFromSoT({chrome,accent}, dims)`: the TokenPath union enumerated from `colours.ts` roles (camelCased) + `dimensions.ts` keys, in the tokens.ts emit order — NO `classifyAll`. (3) **[`tokens-parser.js`](../scripts/tokens-parser.js)** — assemble `resolved` from the TS resolvers (no cascade walk) · token-paths from the SoT · **dead `resolveSemanticCrossProduct` import + re-export removed** (P11 cleanup · still exported from `semantic.js` for the tests). (4) **[`semantic.js`](../scripts/parsers/semantic.js)** — `emitTokensTs` header text updated (dimensions ← `dimensions.ts`). (5) **[`build/tokens.ts`](../packages/spec/build/tokens.ts)** + **[`build/token-paths.ts`](../packages/spec/build/token-paths.ts)** — regenerated (header-only delta). (6) **[`dimension-tokens.test.js`](../scripts/dimension-tokens.test.js)** (new · 8 guards) — A: `resolveDimensionTokens` ≡ a hand-restated design oracle (non-tautological) + context-invariance + `resolveDimLeaf` exhaustiveness · B: committed `tokens.ts` dims ≡ the SoT resolver · C: the union ≡ a hand-restated oracle + the committed file · **D (structural · round-trip dead): corrupt `--nuri-space-md` in the cascade CSS → the old walk moves but `resolveDimensionTokens` is unmoved + the emitters take only TS SoTs (arity guard).** (7) **[`tokens-parser.test.js`](../scripts/tokens-parser.test.js)** — `resolveSemanticCrossProduct` re-pointed to its `semantic.js` source (the barrel stopped re-exporting it). **interaction.ts — STOPPED + surfaced (no SoT invented):** `pressScale 0.97`/`disabledOpacity 0.4` have NO TS home — they live ONLY in `styles/tokens-primitive.css`; `interactive-effects.ts` holds the token PATHS + gate names, not the values. **Its TS-sourcing is the FIRST STEP of 3b·2b** (a 2-constant micro-SoT in `pipeline/` → re-source byte-identical → then the relocation), NOT a separate micro-slice. **Gates green LOCALLY:** spec **60/60** (52 + 8 new) + `build/`+`styles/` byte-identical · prototype 31/31 + styles byte-identical · doc 3/3 + generated byte-identical · **rn render-smoke 29/29 + 7 snapshots BYTE-IDENTICAL** · tsc 0 (rn + expo). **Mutation reproduced (coordinator-verified):** `dimensions.ts` `space.sm` 6→36 → `tokens.ts` `sm:36` + the harness bit · reverted clean. **Next — 3b·2b (the IRREVERSIBLE relocation `build/tokens.ts`→`@nuri/rn`):** step 1 = the `interaction.ts` micro-SoT (above) · **A/B = COMMITTED** for 3b·2b (the resolved-semantic stays committed-in-rn, not gitignored — keep the diff-review gate · §35 survives per-projection) · **two seams 3b·2b INHERITS:** (i) the leaky relative `pipeline/descriptors/schema.ts → ../../build/tokens` import (the 3a residue · the schema's tokens-import rewritten at emit but the SOURCE still reaches cross-tree) · (ii) the cross-package **build-order/gate** (codegen must write `@nuri/rn`'s generated dir before rn builds · extends the N+53 root-build-order fix · the gate must cover rn's generated dir). See [`roadmap/N+60.md`](./N+60.md) · [`decisionlog.md` §80 / §34 / §35](../decisionlog.md) · [`docs/projection-model.md` §4–§6](../docs/projection-model.md).

N+59 · **Slice 3b·1 — RN resolved-semantic: flatten-from-TS + the two-layer accent reshape (in place)** — the first sub-slice of the big **Slice 3b** haul: re-source the COLOUR arm of `build/tokens.ts` by flattening `pipeline/colours.ts` DIRECTLY (ref→hex · NO TS→CSS→TS round-trip) and reshape `accent` to the two-layer accent-major form (`Record<Accent, { role: hex \| {light,dark} }>` · the duplication GONE), staying **in place** in `spec/build/` (the relocation to `@nuri/rn` is 3b·2) (on branch `feat/n59-resolved-semantic-reshape` · base `main` @ `a72e354` [after [#86](https://github.com/nuri-com/nuri-design-system/pull/86) · N+58 docs merged] · [decision 80](../decisionlog.md) · [`docs/projection-model.md` §3](../docs/projection-model.md) [layered substitution] · [§63](../decisionlog.md) · **no new decision** · **no value change** — every resolved hex is exactly what it was). The colour model is **layered substitution** (accent upstream of mode · the runtime composes `chrome[mode] ⊕ accent[accent][mode]`), NOT a materialized cross-product — so chrome stays theme-major + byte-identical and accent collapses its byte-copied solid/solid-pressed/on-solid to single flat hexes (P4-frozen) while the theme-adapting roles stay `{light,dark}` pairs. **Sweep correction — the grounding (+ the sub-agent sweep) said "only `runtimeTokens` + its type"; the `doc` gate refuted it**: `@nuri/doc`'s `makeColorResolver` ([docs.js:154](../packages/doc/pipeline/docs.js)) indexed the accent cross-product (`accent[a][theme][leaf]`) and broke hard — folded in (two-layer collapse · generated docs byte-identical · `foundations-ir.js` was already SoT-shape-aware). (1) **[`colour-tokens.js`](../scripts/parsers/colour-tokens.js)** (new) — `resolveColourTokens`: ref→hex flatten via `loadColours`/`loadSemanticColours` → a chrome cross-product node (accent-invariant · merged over the CSS-walk so the generic emit stays byte-identical) + the accent two-layer table (flat-vs-pair read STRAIGHT from `colours.ts`'s authored shape). (2) **[`semantic.js`](../scripts/parsers/semantic.js)** — `emitTokensTs(resolved, rules, {accentTwoLayer})` · `fmtAccentTwoLayer` + the conditional colour-source header; the legacy all-CSS-walked path is intact when the opt is absent (the test API). (3) **[`tokens-parser.js`](../scripts/tokens-parser.js)** — merge the chrome hexes over `resolved`, pass the accent two-layer; `space`/`size`/`radius`/`type` stay CSS-walked. (4) **[`build/tokens.ts`](../packages/spec/build/tokens.ts)** — regenerated (accent two-layer · **chrome + scales + type byte-identical**). (5) **[`theme.tsx`](../packages/rn/theme.tsx)** — `runtimeTokens` per-role resolve (`typeof v === 'string' ? v : v[mode]`) + the `AccentSlice`/`RuntimeTokens.accent` type (no longer `typeof accentTokens.lilac.light`) · downstream (`resolveToken` · `factory/theme.ts`) UNCHANGED. (6) **[`docs.js`](../packages/doc/pipeline/docs.js)** — the folded-in consumer. (7) **[`colour-semantic.test.js`](../scripts/colour-semantic.test.js)** — Guard E: the composed `(accent,mode)` slice ≡ the SAME independent hand-restated oracle (non-tautological · the emitter is not the oracle) + a flat-vs-pair-tracks-SoT guard. (8) **[`tokens-parser.test.js`](../scripts/tokens-parser.test.js)** — the leaf-render check accepts the `{ light, dark }` pair value shape. (9) **[`resolve.test.ts`](../packages/rn/factory/__tests__/resolve.test.ts)** — an `acc()` helper resolves the two-layer oracle from the contract. **Gates green LOCALLY:** spec 52/52 + idempotent build (only `tokens.ts` changed in `build/`+`styles/`) · prototype 31/31 + styles byte-identical · doc 3/3 + generated byte-identical · **rn render-smoke 29/29 + 7 snapshots BYTE-IDENTICAL** (the core equivalence proof · composed values unchanged) · tsc 0 (rn + expo). **Mutation reproduced:** `colours.ts` `lilac.solid` `lilac.9.light`→`lilac.10.light` flipped `tokens.ts` `#beaaff`→`#b39ff3` + broke 3 render-smoke snapshots (proves `colours.ts` is now the source for `tokens.ts`) · reverted · no residue. Next: **3b·2** (relocate `build/tokens.ts` → `@nuri/rn` · kill `build/` + the round-trip for `token-paths`/`token-vars`/`tokens-semantic.css` · the build-order). See [`roadmap/N+59.md`](./N+59.md) · [`decisionlog.md` §80 / §63](../decisionlog.md) · [`docs/projection-model.md` §3](../docs/projection-model.md).

N+57 · **Slice 3a — drop the verbatim descriptor `.ts` copies (rn imports the source)** — the smallest, first sub-slice of **Slice 3** (`@nuri/spec` → data-only) · kill the 3 `build/descriptors/<name>.ts` passthrough copies and repoint their `@nuri/rn` exports subpaths to the AUTHORED source at `pipeline/descriptors/` (byte-identical data · pure import-target relocation) (on branch `feat/n57-descriptor-ts-drop` · base `main` @ `9d5cd3f` [after [#84](https://github.com/nuri-com/nuri-design-system/pull/84) merged] · [decision 80](../decisionlog.md) · [`docs/projection-model.md` §8](../docs/projection-model.md) · [§69](../decisionlog.md) · **no new decision** · **[decision 2](../decisionlog.md) UNTOUCHED**). **Grounding correction — the brief's "schema = scope creep / type-neutral" premise was WRONG**: dropping the descriptor `.ts` copies FORCES the schema export to follow (a TS type-identity coupling). The factory infers `A` from `Descriptor<A>` whose `Variants<A> = { [Axis in keyof A]: { [Value in A[Axis]]: PartMap } }`; inverting that nested mapped type relies on TS's homomorphic reverse inference, which only fires when the descriptor OBJECT and the consuming `Descriptor<A>` resolve to ONE `schema.ts`. Moving the objects to `pipeline/` (their `./schema` → `pipeline/…/schema.ts`) while `createNuriComponent`/`contract.ts` kept `build/…/schema.ts` → two declarations → `A` collapses to bare `Axes` → every factory component's props degrade to `{ [x: string]: string \| undefined }` (4 rn tsc errors · expo inherits). So **3a's true minimal scope = the 3 descriptor copies + the schema export pointer** — the schema export is repointed to the pipeline source too (export-only · `build/descriptors/schema.ts` stays EMITTED + drift-guarded · `emitSchemaTs` untouched · coordinator-ratified · still §80). (1) **[`package.json`](../packages/spec/package.json)** — 4 descriptor subpaths → `pipeline/descriptors/` (the 3 + schema) · `"//"` note records the coupling. (2) **[`tokens-parser.js`](../scripts/tokens-parser.js)** — the emit loop drops the `.ts`-copy write (keeps the `.js` twin + the schema emit) · comment + build-log updated · `emitDescriptorTsFromSource` stays defined/exported (matches the dead-but-kept re-export pattern). (3) **`git rm`** the 3 copies (160 lines · `schema.ts` + the 3 `.js` twins stay). (4) **[`docs-drift.test.js`](../scripts/docs-drift.test.js)** — Guard D drops the per-descriptor `.ts` assertion (+ its dead import) · keeps the schema `.ts`, the per-descriptor `.js`, Guard F, the IR pins. **Sweep clean**: the only `.ts`-copy consumer was the rn exports subpath · all other `build/descriptors` refs are the `.js` twins (prototype recipes · factory · doc staging) or prose · doc's `stage.mjs` copies the dir but imports only `${name}.js` + its staged tree is gitignored. **Two seams handed to 3b**: (a) `build/descriptors/schema.ts` is now a transitional orphan-emit (still generated + drift-guarded · imported by nothing · retires with the tokens move) · (b) `pipeline/descriptors/schema.ts`'s `../../build/tokens` relative is the **leaky seam** 3b's tokens relocation must resolve (3b's schema work shrinks to: retire the build/schema.ts emit + fix that relative). **Gates green LOCALLY:** `git diff build/ styles/` = exactly the 3 deletions (codegen idempotent · schema.ts retained) · spec 50/50 · prototype 31/31 · doc 3/3 · rn render-smoke 29/29 + 7 snapshots (byte-identical data) · tsc 0 (rn + expo). Non-visual slice (build tooling · the proof is the gates). Next: **3b** (the tokens move) → 3c (the `.js` twins → `@nuri/prototype`) → 3d (the dir renames). See [`roadmap/N+57.md`](./N+57.md) · [`decisionlog.md` §80 / §69 / §35](../decisionlog.md).

N+56 · **Slice 2 — add the `orange` accent (+ generify the last accent-hardcoded spots)** — add a second non-neutral accent, `orange`, as DATA so adding an accent = ramp + `accent.orange` object + one `ACCENTS` entry (N+55's promise · "add data, not edit an emitter") (on branch `feat/n56-accent-orange` · base `main` @ `d7fcfd2` [after [#83](https://github.com/nuri-com/nuri-design-system/pull/83) merged] · [decision 80](../decisionlog.md) · [`docs/projection-model.md` §8](../docs/projection-model.md) slice 2 · **[decision 2](../decisionlog.md) UNTOUCHED** — orange is new data, existing accents byte-identical). **Grounding correction**: the brief said ONE accent-hardcoded spot (`buildSemanticCascade`); the tree had **four** — two emitters + two test oracles. Each generified the same way (loop the non-neutral accents from the data). (1) **[`colours.ts`](../packages/spec/pipeline/colours.ts)** — the 12-step `orange` ramp (Radix from `#ff8c5a` · 9/10 frozen) + `accent.orange` (mirrors lilac: fg + bg-subtle pairs theme-adapt · solid/solid-pressed/on-solid P4-frozen flat). (2) **[`tokens-primitive.css`](../packages/spec/styles/tokens-primitive.css)** — the `--nuri-color-orange-*` block hand-added (the colour emit is an in-place passthrough · sets existing decls · can't ADD them · the build then writes the SoT values byte-identical). (3) **[`colour-css.js`](../scripts/parsers/colour-css.js)** — `loadColours`/`primitiveColourMap` GENERIFIED: accent ramps collected by the `accent`-matrix keys (`accentScales`) + looped · no per-accent line. (4) **[`semantic.js`](../scripts/parsers/semantic.js)** — `ACCENTS += 'orange'` · `selectorMatches` generic over the selector's accent (fixes a LATENT clobber: an `[data-accent="orange"]` block matched EVERY accent under the old `neutralOk`/`lilacOk` and, later in source order, would have overwritten lilac). (5) **[`semantic-css.js`](../scripts/parsers/semantic-css.js)** — `buildSemanticCascade` loops the non-neutral accents: 5 fixed neutral blocks + 3 per accent (i-th at 5+2i/6+2i/(6+2i)b · lilac 5/6/6b **byte-identical** · orange 7/8/8b additive · PARTIAL-per-P4 dark blocks fall out of the data). (6) **[`colour-semantic.test.js`](../scripts/colour-semantic.test.js)** — selectors + shape/self-scope/combinator guards derived from the non-neutral accent keys (auto-cover orange) · orange oracle cells (frozen solid + adapting fg) · `sotScales` spreads `accentScales`. (7) **[`tokens-parser.test.js`](../scripts/tokens-parser.test.js)** — the cross-product loop iterates the accents the oracle DECLARES per cell (chrome/dimension stay accent-invariant · the 6 accent-varying cells pin orange) · `orange` in `RESERVED_COLOR_SCALES` (full-ramp-ships · P11). (8) **[`demo.html`](../packages/playground/pages/demo.html)** — an `Orange` accent button. **Gates green LOCALLY:** neutral + lilac byte-identical (`tokens-semantic.css` zero removed lines · `build/tokens.ts` only the `Accent` widen) · orange additive (7/8/8b · the orange column · 24 `tokens.json` primitives) · spec 50/50 · prototype 31/31 + `styles/` byte-identical (no ripple) · doc 3/3 + `generated/` byte-identical (doc reads neutral/lilac · orange unread) · rn 29 + 7 snapshots byte-identical · tsc 0 (rn + expo). **Render gate** (`demo.html`): the Orange button resolves light (`accent-solid #ff8c5a` · `accent-fg #5e280f`) + dark (`accent-solid #ff8c5a` frozen · `accent-fg #f9d6c8` adapts · `bg-subtle #361a0e`) · console clean. **Mutation reproduced:** drop orange from the matrix + `ACCENTS` → `tokens-semantic.css` identical to pre-orange `main`, `Accent` back to `neutral|lilac` (orange lives only in data · no emitter names it · reverted). Next: **slice 3** (`spec` data-only · the RN resolver + provider). See [`roadmap/N+56.md`](./N+56.md) · [`decisionlog.md` §80 / §72 / §63 / §2](../decisionlog.md).

N+55 · **Slice 1 — the accent reshape (role-major → accent-major)** — reshape the colour semantic accent layer from role-major (`accent[role][accentName][theme]` · accent names baked into the type) to **accent-major** (each accent ONE object · each role a flat `string` ref [theme-invariant · the P4-frozen brand] or a `{light,dark}` pair [theme-adapting · `dark.ref ≠ light.ref`] · names OUT of the type → `Record<string, …>`), so slice 2 (adding accents) is "add one object," not "edit six role entries + widen the type" (on branch `feat/n55-accent-reshape` · base `main` @ `765d5c3` · [decision 80](../decisionlog.md) · [`docs/projection-model.md` §8](../docs/projection-model.md) slice 1 · the FIRST execution slice of the projection model). **Behaviour-preserving — byte-identical** (`tokens-semantic.css` + `build/tokens.ts` re-emit unchanged · `build/tokens.ts` is CSS-walked so it follows · RN snapshots byte-identical · **[decision 2](../decisionlog.md) UNTOUCHED** — the accent SEMANTIC shape moved, no value/SoT-direction changed). (1) **[`colours.ts`](../packages/spec/pipeline/colours.ts)** — `type AccentLeaf` → `type AccentRole = string | {light,dark}` · role order preserved (the emitted decls stay byte-identical) · neutral = 6 pairs · lilac = 3 pairs + 3 flat (the P4-frozen solid/solid-pressed/on-solid) · refs verbatim · chrome + ramps + `refToVar` untouched. (2) **[`semantic-css.js`](../scripts/parsers/semantic-css.js)** — `accentDecls` reads `accent[accentName]` · the `onlyChanged` filter is now `typeof role !== 'string'` (flat = never a dark redeclaration · replaces `dark.ref !== light.ref`) · the bare ref wrapped `{ref}` so `refToVar` is unchanged. (3) **[`tokens-parser.js`](../scripts/tokens-parser.js)** build-log count adapted ("2 accents × 6 roles × {light,dark}" · log only). (4) **[`colour-semantic.test.js`](../scripts/colour-semantic.test.js)** `sotRefFor` re-pointed · the independent matrix oracle UNCHANGED. (5) **[`foundations-ir.js`](../packages/doc/pipeline/foundations-ir.js)** — the `@nuri/doc` Foundations emitter (a CI-gated cross-package consumer the brief didn't enumerate · found by sweep) re-pointed `cols.accent` → `cols.accent.neutral` (default scope) · generated Markdown byte-identical. **Gates green LOCALLY:** byte-identical `build/`+`styles/` · spec 50/50 · prototype 31/31 + `styles/` byte-identical · doc 3/3 + `generated/` byte-identical · rn 29 + 7 snapshots byte-identical · tsc 0 (rn + expo + standalone `colours.ts` satisfies). **Non-tautology reproduced:** flat→pair flip → Guards A/B bite (the role wrongly enters dark blocks 6/6b) · ref perturb → Guard C (the independent oracle · SoT path) bites while the live-CSS leg stays green (the oracle is SoT-independent) · both reverted. **Open question resolved: byte-identical** (RN contract untouched · the RN reshape + provider land at slice 3). Next: **slice 2** (add the 1–2 new accents · one object + one ramp each) → **slice 3** (`spec` data-only · the RN resolver + provider). See [`roadmap/N+55.md`](./N+55.md) · [`decisionlog.md` §80 / §72 / §63 / §2](../decisionlog.md).

N+54 · **the projection-model design lock (doc + ledger only · no code)** — lock the concrete architecture after the [N+52](./N+52.md)/[N+53](./N+53.md) inversion as a design doc + a ledger pointer (on branch `feat/n54-projection-model-doc` · base `main` @ `89e3d93` · [decision 80](../decisionlog.md) · the concrete, gated form of [decision 70](../decisionlog.md) · **docs + ledger only · [decision 2](../decisionlog.md) UNTOUCHED**). [`docs/projection-model.md`](../docs/projection-model.md): **`@nuri/spec` is PURE DATA** (`tokens`/`axes`/`components`/`icons` · no codegen, no deps, no generated, no logic); every consumer is a **projection** that resolves spec's data and owns that resolution — **web** (CSS → `@nuri/prototype` · browser resolves at runtime) + **RN** (resolved-semantic hex → `@nuri/rn` · refs flattened at build · runtime selects only the theme/accent slice · no TS→CSS→TS round-trip · primitives never shipped). **Colour is the only context-variant axis** (theme ⊥ accent · accent-major refs into Radix ramps `{light,dark}` · chrome theme-only) → the RN provider is colour-only, opt-in (Unistyles a perf detail). **Dies**: `build/` in `spec` · the round-trip · the descriptor `.ts` copies · the fossil dirs. The slice plan ([§8](../docs/projection-model.md)): accent reshape → add accents → `spec` data-only → catalog. [§80](../decisionlog.md) **supersedes** [decision 35](../decisionlog.md) (`build/` committed in `spec` — the resolved contract now leaves `spec`) + the model docs `resolver-model`/`emit-model`/`composition-model` (Phase-5 purge). **No code touched** — `npm test --workspaces` stays green. Next: **Phase 4·4** (`spec` data-only · the slice plan) · Phase 5 ledger purge → EXIT. See [`roadmap/convergence.md`](./convergence.md) Phase 4·4 / 5 · [`decisionlog.md` §80 / §70 / §63 / §35 / §2](../decisionlog.md).

N+53 · **Phase 4·3 — relocate the codegen out of `@nuri/spec` (`spec` → no-deps / no-scripts)** — move the `postcss`-using token codegen (the orchestrator + the 12 emitters + the 6 drift-guard tests) OUT of `@nuri/spec` into a root `scripts/` dir, so the spec package ends with **no `devDependencies` and no `build`/`test` scripts** — pure data its consumers import, regenerated by an external tool (on branch `feat/n53-codegen-relocation` · base `main` @ `376e646` · [decision 79](../decisionlog.md) · the structural EXIT-path step of [decision 68](../decisionlog.md)'s Phase 4 · **ORGANIZATIONAL · [decision 2](../decisionlog.md) UNTOUCHED** · as-built [`N+53.md`](./N+53.md)). **Behaviour-preserving — the generator's LOCATION moved, its OUTPUT did not**: `build/*` + `styles/*` + `lib/components/icon/icons.js` re-emit **byte-identical** from the new home. (1) **The mechanical trick = a one-line re-anchor** — VERIFIED zero `import … from '*.ts'` in the codegen (all SoT reads go via `REPO_ROOT`-relative path constants + data-URL `stripTypes` loads), and every output path is `REPO_ROOT`-relative, so `REPO_ROOT = resolve(__dirname, '..')` → `resolve(__dirname, '../packages/spec')` keeps every path constant resolving to the same files. The `git mv` preserved the `parsers/` subdir → the inter-emitter imports are **pure renames** (12 `parsers/*` = `R`, the 7 re-anchored files = `RM`). The 8 `.ts` SoTs + `descriptors/*.ts` STAYED in `pipeline/` (the `./resolve-map`/`./dimensions`/… subpath exports keep resolving · no consumer-path churn). (2) **The `@nuri/prototype` → `scripts/parsers/semantic.js` Guard-C coupling** (the one thing the brief's anti-goal didn't anticipate · operator-approved) — 2 prototype namespace-CSS freshness tests reach across the boundary into the colour-cascade resolver (`semantic.js`) as Guard C's value oracle; `semantic.js` has 6 in-codegen importers so it moved with the codegen, and the 2 imports were repointed `../../spec/pipeline/parsers/` → `../../../scripts/parsers/` (the only prototype touch · test-import-only · no build-logic change · `css-preview.js` reads only its own `./parsers/*` + spec's exported SoTs · unaffected · **recorded in [§79](../decisionlog.md)** so a future move knows). (3) **The root-build-order fix landed** (the deferred [N+41](./N+41-prototype-carve.md) footgun discharged) — root `build` is `node scripts/tokens-parser.js && npm run build --workspaces --if-present` (codegen FIRST · then `prototype` reads the fresh `tokens-semantic.css` · DAG `prototype → spec`) · root `test` runs `node --test scripts/*.test.js && npm test --workspaces` · `type:module` + `postcss` added to the root (lockfile **5/5** · pure declaration relocation spec→root · RN dual-tree untouched). (4) **CI `spec` job re-pointed, name preserved** (a required check · the [N+28](./N+28-A2.5.md) A1 precedent) — runs `node --test scripts/*.test.js` + `node scripts/tokens-parser.js` + `git diff --exit-code packages/spec/build/ packages/spec/styles/` (`styles/` ADDED · regenerated in place). Gates green LOCALLY: **the relocated codegen** byte-identical (`build/`+`styles/`+`icons.js`) · **relocated guards 50/50** + **mutation-proven from the new home** (`dimensions.ts` `space.xs {ref:4}→{ref:6}` fails Guards A/B/C · reverted) · **`@nuri/spec` has no `scripts`/`devDependencies` keys** · **prototype 31/31** (repointed imports load) + `styles/` byte-identical · **doc 3/3** + `generated/` byte-identical · **rn 29 + 7 snapshots** byte-identical · **tsc 0** (rn + expo) · root `npm run build` order verified end-to-end. **[decision 2](../decisionlog.md) UNTOUCHED** (no SoT moved · no CSS authored/generated · the residual hand-CSS primitives stay hand-CSS). Next: **Phase 4·3 residues** (font-size/weight primitives · `--nuri-border-*` · fonts · durations → TS · optional polish · orthogonal to dropping postcss) · the descriptor long-tail · **Phase 5** (the ledger purge + `MEMORY.md`) → EXIT. See [`roadmap/N+53.md`](./N+53.md) · [`decisionlog.md` §79 / §68 / §35 / §2](../decisionlog.md).

N+52 · **Phase 4·2 — the type-composite flip (CSS→TS · a self-contained inline text-styles table)** — invert the type scale from CSS-SoT to TS-SoT, the value flip [§77](../decisionlog.md) explicitly deferred to phase 4 (on branch `feat/n52-type-scale-flip` · base `main` @ `c43e03c` · [decision 78](../decisionlog.md) · executing [decision 70](../decisionlog.md) on a token residue · as-built [`N+52.md`](./N+52.md)). The six `--nuri-type-*` step composites are now authored once in [`pipeline/typography.ts`](../packages/spec/pipeline/typography.ts) as a **self-contained, readable INLINE table of text styles** (`md: { fontSize: 17, lineHeight: 1.29, letterSpacing: -0.02, weight: 'regular' }` · deliberately NOT the [`dimensions.ts`](../packages/spec/pipeline/dimensions.ts) `{ref}` indirection — six steps, the value is seeing the whole style at once) and the build *writes them into* `styles/tokens-primitive.css` ([`parsers/type-css.js`](../packages/spec/pipeline/parsers/type-css.js) · Slice 0 · in-place postcss rewrite), **DE-REFERENCED end to end**: `--nuri-type-*-size` `var(--nuri-font-size-N)` → the rem literal (px÷16 · `17`→`1.0625rem`) · `--nuri-type-*-weight` `var(--nuri-font-weight-X)` → the literal (`regular`→`400`). So the `--nuri-font-size-*`/`--nuri-font-weight-*` PRIMITIVES go back to being a foundational hand-CSS layer (`shell.css`'s concern) — **the fake type↔shell coupling dissolves.** [`parsers/type.js`](../packages/spec/pipeline/parsers/type.js) is re-sourced onto the SoT (fontSize/lineHeight/letterSpacing straight from the table · only the weight name resolved against the primitive · one source, two readers). **Gate = computed-equivalent** (the [§77](../decisionlog.md)/N+45 posture, not §71 byte-identity): `build/tokens.ts` `type` + RN snapshots (29+7) **byte-identical** (resolved values unchanged); the ONLY changed bytes are the `--nuri-type-*` block (var→inline · incl. one cosmetic `xl` `line-height 1.20`→`1.2`); everything else in `build/*` + `styles/*` + prototype + doc byte-identical. **Scope = the type COMPOSITE only** (the [§77](../decisionlog.md) open question, resolved by grounding): the font primitives stay hand-CSS (a Phase 4·3 residue) — de-referencing left `--nuri-font-size-30`/`-57` (the only steps `shell.css` doesn't use) with no consumer → RESERVED in the guardrail (P11 · membership re-decided at 4·3); `--nuri-font-weight-regular` stays consumed by `shell.css` (not orphaned · NOT deleted); the `2xl` gap stays reserved. The drift guard ([`tokens-parser.test.js`](../packages/spec/pipeline/tokens-parser.test.js)) is re-pointed at the SoT (the N+31 dimension-cascade harness pattern · an independent restated-scale oracle resolved BOTH through the SoT and the live inline CSS + the in-place re-emit byte-identity + the `tokens.ts` re-emit · non-tautological · mutation-proven). Gates green LOCALLY: **spec 50/50** · `npm run build -w @nuri/spec` → `git diff packages/spec/build/` **byte-identical** + `styles/tokens-primitive.css` diff = the `--nuri-type-*` block only · **rn 29/29 + 7 snapshots** byte-identical · **tsc 0** (rn + expo) · **prototype 31** + `styles/` byte-identical · **doc 3/3** + generated foundations byte-identical. **[decision 2](../decisionlog.md) reversed for the type composite** (the token VOCABULARIES — dimensions ✓ · colour ✓ · type ✓ — are TS-authored; the inversion is NOT complete — font/border/font-family/duration primitives stay hand-CSS). Deferred (accepted): the font-size/weight primitive flip (re-decides 30/57) + border · font families → TS · the codegen move + dropping `postcss` (**Phase 4·3**). Next: **Phase 4·3** (the token residues + the codegen move → `@nuri/spec` no-deps/no-scripts) · the descriptor long-tail · phase-5 ledger purge · phase-6 rich-demo rebuild. See [`roadmap/N+52.md`](./N+52.md) · [`decisionlog.md` §78 / §77 / §71 / §70 / §2](../decisionlog.md).

N+51 · **Phase 4·1 — icons-as-folder (the SVG folder is the SoT · the registry GENERATED · `@phosphor` dropped) + the size closed into the box axis** — re-model the icon system so adding an icon = **dropping a `.svg`** in `packages/spec/icons/`; the web registry, the `IconName` type, and the RN reader are all GENERATED from the folder (on branch `feat/n51-icons-folder` · base `main` @ `eda0ef1` · **no new decision** — [decision 38](../decisionlog.md) [icon = a closed enum of named SVG assets · bespoke · `currentColor`] + [decision 48](../decisionlog.md) [one registry, two readers] + [decision 35](../decisionlog.md) [build/ committed · re-emit gate] cover it · the weight-coupling §38 defined is what we RETIRE · as-built [`N+51.md`](./N+51.md)). Replaces the hand-curated phosphor registry (17 glyphs × 3 weights) with the operator's new model: **one drawing per glyph · NO weights · `currentColor` · md=24 / sm=18** (NOT byte-identical — glyph/registry/render change by design). (1) **The SoT folder** ([`packages/spec/icons/*.svg`](../packages/spec/icons) · `apple`·`bitcoin`·`card`·`euro` seed · `viewBox 0 0 32 32` · filename = name) — the only hand-touched icon surface; the catalog grows by dropping files, no session. (2) **The generator** ([`parsers/icons.js`](../packages/spec/pipeline/parsers/icons.js) · rewritten · spec pipeline · NO postcss): `readIcons(dir)` + `extractIconMarkup` (inner `<path>`(s) verbatim · preserves `fill-rule`/`clip-rule` · normalizes every `fill`→`currentColor` · asserts `viewBox 0 0 32 32` LOUD) → `ICONS = { name: '<path…/>' }` (no weights · `ICON_WEIGHTS` retired) → `emitIconsJs` + `emitIconsTs` emit BOTH readers: [`lib/components/icon/icons.js`](../packages/spec/lib/components/icon/icons.js) (web · now GENERATED) + [`build/icons.ts`](../packages/spec/build/icons.ts) (RN · `IconName` union + `Record<IconName, string>` · reshaped · `IconWeight` gone) · [`tokens-parser.js`](../packages/spec/pipeline/tokens-parser.js) Slice 6 READS the folder. (3) **The web element** ([`prototype/primitives/icon.js`](../packages/prototype/primitives/icon.js)) — weight-coupling dropped · the `fill` attr REMOVED (resolves the deferred icon-arc `fill` first-bump gap · operator-confirmed) · re-wrapped in `<svg viewBox="0 0 32 32" fill="currentColor">`. (4) **The size, closed into the SHARED box axis** (the coordinator's re-checkpoint ask · operator-ENDORSED · NOT a bespoke icon rule): `size.sm` `{ref:28}→{ref:24}` in [`dimensions.ts`](../packages/spec/pipeline/dimensions.ts) (`--nuri-size-sm→px-24` · `px-28` NOT orphaned — `shell.css` uses it 9× · no active component used `size.sm`); the standalone `<nuri-icon>` maps its public `size`→a size leaf and applies `nuri-box`+`data-width`/`data-height` (it RESPECTS a host-pinned box) · `icon.css` keeps ONLY the primitive setup (display/`currentColor`/svg-fill) + the skeleton (now `--nuri-size-sm/xs`) · the icon-avatar glyph gains `icon:{box:{width:'sm',height:'sm'}}` ([`descriptors/icon-avatar.ts`](../packages/spec/pipeline/descriptors/icon-avatar.ts)) applied by the factory on BOTH targets (web `renderIcon` → `nuri-box`+`data-*` · RN `case 'icon'` drives the consumer-glyph `{color, dimension}`). **NAMING OFFSET**: icon `md` ↔ size `sm` (24) · icon `sm` ↔ size `xs` (18). (5) **`@phosphor-icons/core` DROPPED** ([`spec/package.json`](../packages/spec/package.json) · lockfile −8 · RN tree untouched · `postcss` STAYS — Phase 4·3). (6) **Guards re-pointed**: the icon sync test ([`tokens-parser.test.js`](../packages/spec/pipeline/tokens-parser.test.js)) is now the **folder → registry ROUND-TRIP** (both committed readers re-emit ≡ folder · every fill `currentColor` · `IconWeight` must not return) · `size.sm` oracles → `24px` · the `docs-drift` glyph-COUNT assertion RETIRED (the count grows by dropping files · `llms.txt`/`README` de-hardcoded to the folder model). (7) **Demos remapped** to the present glyphs (no old→new map · the demo adapts to the folder): `demo.html` · the doc icon-avatar story · RN `Demo.tsx`/`DemoIcon.tsx` · `factory.test.js`. **DoD demonstrated**: dropped a `dollar.svg` → `npm run build -w @nuri/spec` → `IconName` + both readers regenerated to include it, **zero hand edits**; removed it → back to the seed. Render gate (visual feedback FIRST · `http-server` · `demo.html`): the new glyphs render at **md=24/sm=18** (every glyph `nuri-box`+`data-width=sm` → 24px · sm-probe `data-width=xs` → 18px · `data-size` gone · viewBox 32) inside icon-avatars + standalone · `currentColor` re-colours on theme (`rgb(34,32,19)`→`rgb(240,238,227)`) AND accent (solid avatar rides lilac on-solid `rgb(56,27,106)`) · **console clean** · render pixel-identical before/after the size rework (only the value's SOURCE moved). Gates green LOCALLY: **spec 50/50** · **prototype 31** (24+7) + `styles/` byte-identical · **rn 29 + 7 snapshots** — `render-smoke` delta **ZERO** (no rendered component uses `size.sm` · the consumer glyph absorbs the injected `dimension`) · the ONE expected change `resolve.test.ts.snap` IconAvatar `icon.base: {} → {width:24,height:24}` (the descriptor box in the resolved recipe · not a render) · **tsc 0** (rn + expo) · **doc 3/3** (2 intended generated hunks: `icon-avatar.md` icon-box row · `dimension.md` `size.sm 24px`). **Three durable facts**: `size.sm`=24 (a scale refinement · `px-28` stays a primitive) · the icon sizes via the **box axis** (`<nuri-icon>` is a `nuri-box` consumer · "an icon is a box of a size" · the operator-endorsed call · no bespoke `[data-size]` residue) · the md↔sm / sm↔xs naming OFFSET. **[decision 2](../decisionlog.md) UNTOUCHED** (the icon stays bespoke · NOT a descriptor/token). Deferred (first-bump · accepted): the catalog itself (grows by dropping files) · the codegen-move + dropping `postcss` (**Phase 4·3**) · topbar inset/title-type. Next: **phase-4 token residues** (type-scale · `--nuri-border-*` · fonts → TS) · **Phase 4·3** (codegen move · drop `postcss`) · the descriptor long-tail · phase-5 ledger purge · phase-6 rich-demo rebuild. See [`roadmap/N+51.md`](./N+51.md) · [`decisionlog.md` §38 / §48 / §35](../decisionlog.md).

N+50 · **A6 — the web factory (`defineNuriComponent`) + R1.5 defaults-in-data** — build the missing web mirror of RN's `createNuriComponent` so `<nuri-button>` + `<nuri-icon-avatar>` are GENERATED from their descriptors, not hand-written, and close the no-harness gap the factory has carried since [N+38](./N+38-L3c-flip.md) (on branch `feat/n50-web-factory` · base `main` @ `4b3127d` · **realizes the [§65](../decisionlog.md)/65.5 generic-factory model on web + closes R1.5 with data · no new decision** · as-built [`N+50.md`](./N+50.md)). The goal: **adding a merged-node component = adding data in spec, nothing hand-written.** (1) **R1.5 → the descriptor** ([`schema.ts`](../packages/spec/pipeline/descriptors/schema.ts) · a *versioned* post-freeze envelope add · Guard F pin updated): `Descriptor.defaults?: Partial<Record<string,string>>` (the per-axis PUBLIC default · `button {variant:'soft', size:'md'}` · `icon-avatar {variant:'soft'}`) + `decorative?: boolean` (`icon-avatar` · the [decision 50](../decisionlog.md) a11y flag · honest data, not a hand `aria-hidden`). **BOTH factories read `defaults`** (operator-confirmed at the checkpoint): the web `buildComponent` fallback + RN's `defaultByAxis` now `descriptor.defaults?.[axis] ?? Object.keys(variants[axis])[0]` — correcting RN's latent default bug (`solid`/`sm` → `soft`/`md`) from data, the honest 1:1 web↔RN parity close. **RN snapshot delta = ZERO** (every smoke test passes explicit props · the fix lands in the code path with no churn · cleaner than the brief's hypothesis). (2) **`defineNuriComponent(descriptor, tagName)`** ([`factory.js`](../packages/prototype/factory/factory.js) · the web twin of `createNuriComponent`): derives `observedAttributes` from the axes ∪ `accent` ∪ `disabled`? (iff interactive root) ∪ `name`? (iff icon primary), a generic `HTMLElement` whose `connectedCallback`/`attributeChangedCallback` read attrs → selection + props → `buildComponent` → `replaceChildren`, captures the text label for a text primary, sets `aria-hidden` from `descriptor.decorative`. ONE factory, zero per-component code. (3) **The 2 recipes COLLAPSED** to one-line registrations ([`recipes/button.js`](../packages/prototype/recipes/button.js) · [`recipes/icon-avatar.js`](../packages/prototype/recipes/icon-avatar.js) · the hand `HTMLElement` classes DELETED · −130 lines · keep the primitive self-imports the built tree upgrades into). (4) **The factory harness COMMITTED** ([`factory.test.js`](../packages/prototype/factory/factory.test.js) · jsdom · 7 tests · `jsdom` added as a conservative non-RN prototype devDep · lockfile **+1/−0** · RN tree untouched · test glob extended to `factory/*.test.js`): `buildComponent` + `defineNuriComponent` teeth — the de-collapsed tree, namespace classes/`data-*`, the **defaults-from-data** (the R1.5 assertion · soft not solid · md not sm), label/glyph routing, disabled reflection, decorative aria-hidden, + a synthetic descriptor proving genericity (a NEW component = a descriptor + one line). **TOPBAR OUT** — it stays the hand `apply-NS-to-host` path (the factory's open-view model can't place positional leading/content/trailing slots and the schema can't encode them · a real model extension reassessed in a separate coordinator brainstorm). Render gate (visual feedback FIRST · `http-server` at the repo root · `demo.html`): byte/behaviour-identical — solid-md bg `rgb(18,17,11)` · 48px · radius 6 · pad 18 (≡ N+38/N+41) · a bare `<nuri-button>` resolves to **soft/md from data** · 3 IconAvatars `aria-hidden` (decorative from data) · accent re-resolves (solid `#12110b`→`#beaaff` lilac) · console CLEAN. Gates green LOCALLY: **spec 50/50** + build twins re-emitted (only the `defaults`/`decorative` delta · doc-gen reads `variants`/`structure`, not the new fields → no leak) · **prototype 31** (24 + 7 harness) + `styles/` byte-identical · **doc 3/3** byte-identical · **rn 29 + 7 snapshots** (snapshot delta ZERO) · **tsc 0** (rn + expo). **[decision 2](../decisionlog.md) UNTOUCHED** (no CSS authored/generated · the namespace CSS is byte-identical). Deferred (first-bump · accepted): RN honoring `decorative` (web honors it now · RN unchanged · no regression — RN's IconAvatar was never aria-hidden) · `icon-avatar` `fill` weight · topbar slot model. Next: **phase-4 token residues** (type-scale · border · fonts → TS) · the descriptor long-tail (descriptors for the other ~16 hand components) · phase-5 ledger purge · phase-6 rich-demo rebuild. See [`roadmap/N+50.md`](./N+50.md) · [`decisionlog.md` §65 / §50 / §74](../decisionlog.md).

N+49 · **A5 — carve `@nuri/playground` (the build-free bench · the 6th & last package)** — extract the playground surface out of `@nuri/spec` into its own workspace, closing the last extraction of [decision 68](../decisionlog.md)'s 6-package split — **the 6-package structure is now REAL** ([convergence §2](./convergence.md) exit CLOSED) (on branch `feat/n49-playground-carve` · base `main` @ `ef33355` · **pure re-plumbing · behaviour-preserving · no new decision** — §68 + [§57](../decisionlog.md) cover it · as-built [`N+49.md`](./N+49.md)). The A3/A4 carve, smaller. (1) **The `@nuri/playground` workspace** (`package.json` · `private` · `type: module` · deps `@nuri/spec` + `@nuri/prototype` · the A4 shape · **no build/test** — build-free HTML) · root `npm install` wired it (a conservative non-RN add · lockfile **+12/−0** · RN tree untouched). (2) **The 4 files MOVED** (`git mv` · 100% renames): `spec/pages/playground/{demo,index}.html` → `packages/playground/pages/*` · `spec/lib/playground/shell.{css,js}` → `packages/playground/lib/*` (**`spec/pages/playground` + `spec/lib/playground` GONE from `@nuri/spec`**). (3) **The path rewire** (the A3 build-free precedent · workspace-relative `../`-paths, NOT `node_modules/@nuri/*`): `../../styles/*` → `../../spec/styles/*` (the token cake) · `../../../prototype/*` → `../../prototype/*` (depth −1 · primitives/namespace CSS/recipes/reset) · `../../lib/playground/shell.*` → `../lib/shell.*` (in-package) · **`shell.js`'s ROOT split** (`PKG = ../` for `INDEX_HREF` · `REPO_ROOT = ../../../` for the DS-entry `DS_HREF`). (4) **The dangling `state.js` ref RESOLVED** (not cross-wired to doc): `index.html` loaded `../../lib/docs/state.js` which the A4 carve moved into `@nuri/doc` (already a 404 on `main`) — **dropped the dep**; `index.html` now self-pins its chrome inline like `demo.html` (`data-theme=light` · `data-neutral=cream` · the §57 fixed surface). Depending on `@nuri/doc` would INVERT the DAG (doc is a consumer); the DAG stays **`playground → prototype → spec`**. Render gate (visual feedback FIRST · both pages build-free · `http-server` at the repo root ≡ opening the file): **demo.html** — all 7 custom elements DEFINED (console CLEAN) · the factory de-collapses the button (solid md bg `rgb(18,17,11)` · 48px · radius 6px · padding 18px ≡ N+41/N+42) · IconAvatar SVG paints · cream surface resolves · **theme/accent re-resolve LIVE** (dark body `rgb(18,17,11)` · lilac accent `#12110b`→`#beaaff` · avatar `rgb(18,17,11)`→`rgb(190,170,255)`); **index.html** — `nuri-playground-shell` built (topbar + the Demo-bench card on cream · radius-lg 18px) · **console CLEAN, NO `state.js` 404**. Gates green LOCALLY: `@nuri/spec` `build/*` **byte-identical** (`git diff --exit-code`) · `@nuri/prototype` `styles/` byte-identical · **no consumed asset changed** · **spec 50/50 · prototype 24/24 · doc 3/3 · rn 29 + 7 snapshots · tsc 0** (rn + expo). **[decision 2](../decisionlog.md) UNTOUCHED** (the playground is HTML + the already-flipped runtime · no CSS authored/generated). Deferred (noted, not chased · out of scope): the DS doc-site footer link to the playground (already dangling pre-A5 · a deploy concern) · the frozen archive `my-vault` refs (rebuild at phase 6) · the repo-root `index.html` stale redirect (the N+42 doc-hygiene cascade · the only render-gate 404, from the `/` load, not a playground asset). Next: **A6** (codegen surfaces + the factory harness) · phase-4 token residues · phase-5 ledger purge · phase-6 rich-demo rebuild. See [`roadmap/N+49.md`](./N+49.md) · [`decisionlog.md` §68 / §57](../decisionlog.md).

N+48 · **A4c — the Foundations doc emitter (the token family · §75's 3-family set CLOSED)** — add the **Foundations doc emitter** to `@nuri/doc` + generate the token-vocabulary reference (**colour · dimension · typography**) from `@nuri/spec`'s token SoTs, filling the **Foundations** nav placeholder (on branch `feat/n48-foundations-docs` · base `main` @ `1e7155d` · **doc-driven · additive · read-only · no new decision** — §75 + §70 + §77 cover it · as-built [`N+48.md`](./N+48.md)). The LAST of [decision 75](../decisionlog.md)'s 3 generated-doc families (Components ✓ · Axes ✓ · **Foundations**) — the doc arc COMPLETE. (1) **`foundations-ir.js`** (NEW · the `axis-ir.js` sibling) + the `FOUNDATION_DOCS` manifest — 4 builders: **colour-primitive** ← `colours.ts` (cream + lilac + black/white alpha · theme-fixed LITERAL swatches · only cream live · decision 31) · **colour-semantic** ← `colours.ts` + the N+22 role resolver (chrome/accent role → the `{ref}` cascade pointer + a LIVE `var()` swatch + the resolved hex · the slice the palette AXIS samples) · **dimension** ← `dimensions.ts` + `tokens` (the px primitives + the space/size/radius scales · each leaf → the `px-N` it references OR the `literal` sentinel · + the resolved px) · **typography** ← `tokens.type` + `emphasisWeight` (the 6 size composites + the orthogonal emphasis weight · the de-fused §77 shape · NOT the old `${size}Em` twins · reads the DATA · the scale stays CSS-SoT). (2) **`docs.js` · `emitFoundationPage` + 4 renderers** (`renderDimension`/`renderColourPrimitive`/`renderColourSemantic`/`renderTypographyScale` · reuses `swatch`/`genHeader`/`frontMatter`/`titleFor` · the palette swatch idiom). (3) **`strip.js` — the depth-aware `as const satisfies` extension** (`stripAsConst`): the old line-bounded `[^,;\n]+` arm TRUNCATED at the comma inside `Record<string, Leaf>` (→ `}, Leaf>;` syntax error · the SoTs' table suffix); now scans depth-aware to the depth-0 terminator. **VERIFIED**: loads both new SoTs + all 8 existing subpaths · 0 regressions. (4) **`packages/spec/package.json` — 2 additive exports** (`./dimensions` + `./colours` → `pipeline/*.ts` · the ONLY spec edit · the `token-vars`/A3 precedent · no consumer/guard reads them · `build/` byte-identical). (5) **`build.js` · `buildFoundationDocs`** (loads the SoTs + the SAME `buildDocTokenInputs`/`makeRoleResolver` the component/axis pages use → emits the 4 pages · `buildDocs` concatenates all 3 families). (6) **`foundations.md`** (`has_children: true` · placeholder retired · the cascade description) + **`_config.yml`** (`generated/foundations` → `parent: Foundations` · the byte-identical-nav trick). (7) **Guard G — a 3rd test** (`generated/foundations/*.md` re-emits ≡ committed + per-page pins: a dimension cascade row [`space.md` → `px-12` → `12px`] + `radius.full` literal · a colour-primitive literal swatch [cream + lilac-frozen + alpha] · a semantic role [`bg-canvas` → live var + hex · the INVERSE `accent-solid`] · a type composite + the emphasis note). **Page granularity = 4** (operator's call at the rendered checkpoint · the archived hand split): colour SPLIT into `colour-primitive` (4 ramps · 48 swatches) + `colour-semantic` (chrome/accent · 19 live swatches) + `dimension` + `typography`. Gates green LOCALLY: **doc 3/3** (Guard G · all 3 families) · `npm run build -w @nuri/doc` → the 4 foundations pages emit · **the 8 existing pages byte-identical** · re-emit ≡ committed · the 2 additive exports hold (**spec 50/50** · `git diff packages/spec/build/` byte-identical · **prototype 24/24** · **rn 29/29 + 7 snapshots** · **tsc 0** rn + expo). Render gate (visual feedback FIRST · a throwaway proof mirrors `head_custom.html` · `nuri-doc` :8768 · Jekyll can't run locally): the swatches RESOLVE — literal primitives paint their hex (cream/lilac/alpha), the live `var()` semantics resolve at the default scope (`--nuri-bg-canvas` → `#fffdf2` · `--nuri-accent-solid` → `#12110b` · `--nuri-focus-ring` → `#ae91ff`) AND re-theme on a dark toggle (bg-canvas → `#12110b`) while the literals stay fixed · console clean. **[decision 2](../decisionlog.md) UNTOUCHED** (doc-gen generates NO CSS · the type scale stays CSS-SoT · a phase-4 flip). Next: **A5** (`@nuri/playground`) · **A6** (codegen + the factory harness) · phase-4 token residues · the descriptor long-tail. See [`roadmap/N+48.md`](./N+48.md) · [`decisionlog.md` §75 / §70 / §77 / §31](../decisionlog.md).

N+47 · **axis-doc fan-out — apply the locked grammar to the remaining axes + delete the `effects` bridge** — apply the agnostic `| Input | Web | RN | Value |` grammar [N+46](./N+46.md) LOCKED to the other axes + finish the SoT cleanup the engine arc tracked (on branch `feat/n47-axis-doc-fanout` · base `main` @ `6727b32` · **doc-driven · one inert spec deletion · no new decision** — §76 + §77 + §75 + the N+46 lock cover it · as-built [`N+47.md`](./N+47.md)). This is the session that fully CLOSES the "CSS guide" critique that started the axis-doc arc. (1) **box · stack — the mechanical `CSS → Web` rename** ([`docs.js#renderFields`](../packages/doc/pipeline/docs.js)) · rows byte-identical (still the web spelling) · only the header breaks byte-identity · no IR change. (2) **interactive — re-sourced** `interactiveAxisIr(effects)` → `interactiveAxisIr(opts, webChrome, webOrder)` ([`axis-ir.js`](../packages/doc/pipeline/axis-ir.js)): the agnostic **`## Effects`** table (the 3 `opts` rows · pressColor now appears · **Web** = the assembled selector → decls or `→ palette (:active bg swap)` where the rule is palette's · **RN** spelled from the pure `rn` data `prop ← source` · **Value** = the gate) + the demoted web-only **`## Chrome`** section (the 3 `webChrome` rows · affordance/focus/disabledGuard · mirrors typography's `## Wrapper` · a web-only label note) + the **order note DEMOTED** into `## Chrome` as a 1-line caption (the [N+46](./N+46.md) precedent · still data-derived from `webOrder` · operator-confirmed KEEP at the checkpoint); the `lead` reframed agnostic honoring the web/RN framing (**RN production · web prototyping/doc** · not peers). (3) **the derived `effects` bridge DELETED** — the `effects` export + the `webRules` const + the §50–59 bridge header removed from [`interactive-effects.ts`](../packages/spec/pipeline/interactive-effects.ts) (NO change to `opts`/`webChrome`/`webOrder`) · [`build.js`](../packages/doc/pipeline/build.js) + [`docs-drift.test.js`](../packages/doc/pipeline/docs-drift.test.js) re-pointed. **Inert by construction · VERIFIED against the tree**: only `@nuri/doc` read the export (prototype parses `opts`/`webChrome`/`webOrder` from source text · RN imports `opts` · the factory reads `opts[key].gate`). **palette left byte-identical** (operator-confirmed): it resists the grammar — the colour FUNNEL resolves each role to a TRIPLE (bg/fg/pressed) not a per-target spelling, and is ALREADY agnostic (no "CSS" column · a Web|RN split would duplicate identical resolved values · its role-table shape IS its honest bespoke form · [§73](../decisionlog.md)). So **3 pages break byte-identity (box · stack · interactive), not 4**. Gates green LOCALLY: **doc 2/2** (Guard G) · `npm run build -w @nuri/doc` → **only box/stack/interactive.md change · palette/typography + the 3 component pages byte-identical** · re-emit ≡ committed · the inert spec touch holds (**spec 50/50** · `git diff packages/spec/build/` byte-identical · **`interactive.css` byte-identical** · **prototype 24/24** · **rn 29/29 + 7 snapshots byte-identical** · **tsc 0** rn + expo) · `effects`-export grep clean. Render gate (visual feedback FIRST · pure Markdown · Jekyll can't run locally): the interactive page renders the agnostic opts table (the palette cross-ref) + the labeled web-only chrome + the demoted order caption · box/stack headers read "Web" · console clean. **[decision 2](../decisionlog.md) UNTOUCHED** (doc-gen generates NO CSS). Next: **A4c** (the Foundations type-scale the `size` axis references by name · carry the web/RN framing forward). See [`roadmap/N+47.md`](./N+47.md) · [`decisionlog.md` §76 / §77 / §75 / §73](../decisionlog.md).

N+46 · **typography axis doc — document the de-fused `size`+`emphasis` axis + LOCK the agnostic grammar** — re-point the typography AXIS page (`generated/axes/typography.md`) from the A4b **wrapper-only** page to the de-fused axis ([decision 77](../decisionlog.md)), and LOCK the agnostic axis-doc grammar the fan-out applies to the other four axes (on branch `feat/n46-typography-axis-doc` · base `main` @ `86a306b` · **doc-only · no new decision** — §76 + §77 cover it · as-built [`N+46.md`](./N+46.md)). The page cures the "CSS guide" critique that started the axis-doc arc: the A4b page documented the web-only `muted`/`align` wrapper AS "the typography axis"; the real axis is two orthogonal inputs (`size` + `emphasis` · [§76](../decisionlog.md)/[§77](../decisionlog.md)) with a clean Web↔RN story. (1) **The IR** ([`axis-ir.js`](../packages/doc/pipeline/axis-ir.js) · `typographyAxisIr`) — the **`size`** rows (6 · from `tokens.type`'s now-clean keys · web `[data-type-style="{size}"]` / RN `typeStyle('{size}')`) + the orthogonal **`emphasis`** row (web `[data-type-emphasis]` / RN `typeStyle(size, true)` / → semibold) + the relabeled web-only **wrapper**; the realization spelling is the documented convention (mirrors `styles/typography.css` + `theme.tsx#typeStyle`). (2) **The renderer** ([`docs.js`](../packages/doc/pipeline/docs.js) · `renderTypography`) — `## Size` (6 rows) + `## Emphasis` (1 row) on the locked `| Input | Web | RN | Value |` grammar, then the web-only `## Wrapper` + a 1-line note. (3) **`build.js`** feeds `typeSizes = Object.keys(specTokens.type)`; (4) the `AXIS_DOCS` `src`/`lead` reframed; (5) **Guard G's** typography contract re-pointed (`## Size` · the grammar-header pin · the `## Emphasis`/`## Wrapper` includes · size/emphasis/wrapper cells) + re-emit ≡ the NEW committed page. **The grammar is LOCKED** (operator-confirmed at the rendered checkpoint): `| Input | Web | RN | Value |` — **Web|RN two columns** (the box/stack `CSS`→`Web` rename the fan-out applies) + the composite **referenced** (`type` scale / semibold · not inlined · no A4c overlap). Size = 6 agnostic rows, Emphasis = the single boolean row (**6×1**, not 12 fused `mdEm` keys), the wrapper demoted (its missing RN column = the honest web-only signal). **Fan-out precedent recorded**: the agnostic axis pages **demote web-cascade-order notes** (the §77 source-order tiebreak is a web-cascade mechanic · RN has none · its essential truth is already in the Value column + lead), so interactive's analogous A4b order note demotes the same way when redone (interactive UNTOUCHED here). Gates green LOCALLY + coordinator-reproduced: **doc 2/2** (Guard G) · `npm run build -w @nuri/doc` → **only `typography.md` changes · the other 7 byte-identical** · **spec/prototype/rn/expo UNTOUCHED** (git-clean · `build/*` byte-identical). Render gate (visual feedback FIRST · the rendered page presented at the checkpoint · pure Markdown · no JS/console): the Size+Emphasis tables show the orthogonality · the wrapper labeled web-only. **[decision 2](../decisionlog.md) UNTOUCHED** (doc-gen generates NO CSS). Next: **the FAN-OUT** (apply the grammar to stack/box/palette/interactive · rename `CSS`→`Web` · DELETE the derived `effects` bridge by re-sourcing interactive onto `opts` · [decision 76](../decisionlog.md)) · then **A4c** (the Foundations type-scale the `size` axis references by name). See [`roadmap/N+46.md`](./N+46.md) · [`decisionlog.md` §77 / §76 / §75 / §73](../decisionlog.md).

N+45 · **typography — de-fuse `size`×`emphasis` (orthogonal axis + the data-attr type scale)** — de-fuse the fused `TypeKey` (`mdEm` = `md` + a weight flip · a 12-value enum) into `{ size, emphasis }` as two orthogonal inputs (on branch `feat/n45-typography-defuse` · base `main` @ `b321690` · [decision 77](../decisionlog.md) · as-built [`N+45.md`](./N+45.md)). **Behaviour-preserving — computed-equivalent, NOT byte-identical**: emphasis is a uniform `400→600` weight override (size/line-height/letter-spacing identical across all 6 sizes), so the *shape* de-fuses but no *value* moves. The fusion was the *cause* of the web/RN realization split [N+43](./N+43.md) surfaced (web un-fused `mdEm` via `expandTypeStep`; RN carried it fused); it completes [decision 76](../decisionlog.md)'s typography-axis-is-`size` reclassification into orthogonal `size`+`emphasis` ([P11](../packages/spec/pages/principles.html#p11-parsimony)). (1) **The vocabulary** ([`schema.ts`](../packages/spec/pipeline/descriptors/schema.ts) + [`theme.tsx`](../packages/rn/theme.tsx)) — `TypeKey = TypeSize` (the `${TypeSize}Em` arm dropped) · `TypographyNS = { size?, emphasis? }` (the `box`/`stack` flag precedent). (2) **The data** ([`type.js`](../packages/spec/pipeline/parsers/type.js) + `build/tokens.ts`) — `type` = the **6 size composites** + one `emphasisWeight: '600'` sibling (not 12 fused keys · P11 forbids the per-size split since emphasis is uniform); the per-size `--nuri-type-*-em-weight` primitives retire. (3) **The web realization** ([`typography.css`](../packages/spec/styles/typography.css)) — the 12 `.nuri-type-{step}`/`--em` classes → **6 `[data-type-style="{size}"]` rules + 1 `[data-type-emphasis]` rule** (emitted last · EQUAL specificity `(0,1,0)` → **source order is load-bearing** · the [decision 74](../decisionlog.md) interactive order-guard pattern recurs); the element ([`primitives/typography.js`](../packages/prototype/primitives/typography.js)) reflects `size`/`emphasis` → data-attrs; the factory ([`factory.js`](../packages/prototype/factory/factory.js)) passes them straight through (`expandTypeStep` retired). (4) **The RN realization** ([`resolve.ts`](../packages/rn/factory/resolve.ts) + [`createNuriComponent.tsx`](../packages/rn/factory/createNuriComponent.tsx)) — `typeStyle(size, emphasis)`; `node.type = {size, emphasis}` (`typeStep` reshaped). (5) **The descriptors** ([`composition-button.ts`](../packages/spec/pipeline/descriptors/composition-button.ts)) — `{size:'mdEm'}` → `{size:'md', emphasis:true}` (×3) · the expo demo's 2 call sites updated. (6) **The gate = COMPUTED-EQUIVALENCE** (deliberate API+CSS change · byte-identity does NOT hold): the new node harness [`typography-cascade.test.js`](../packages/spec/pipeline/typography-cascade.test.js) (4 guards · an independently-restated oracle resolved through the live `var()` chain · the N+30/N+31 dimension-cascade pattern · non-tautological · both mutations reproduced) + the real-browser [`typography-defuse-computed-check.html`](../packages/spec/pipeline/typography-defuse-computed-check.html) (**12/12 cells** · the source-order tiebreak real-engine-verified) + the **untouched `render-smoke.snap`** (only `resolve.snap`'s `typeStep` intermediate changed · `-u`) + a `typeStyle(size, true)` == old `{size}Em` oracle test. The de-fused md emphasis computes `17/21.93/600/-0.34` = the RN snapshot (web↔RN parity). **A *versioned* post-freeze schema change** (Guard F `FROZEN_SCHEMA` pin updated · [decision 65](../decisionlog.md) · `TypographyNS` is frozen); `TypeKey` survives as a `= TypeSize` alias. Gates green LOCALLY + coordinator-reproduced: **spec 50/50** (46 + 4 harness guards) · **rn 29/29** + `render-smoke.snap` untouched · **doc 2/2** (only `button.md` regenerated · the axis typography.md unchanged) · **prototype 24/24** · **tsc 0** (rn + expo). **[decision 2](../decisionlog.md) UNTOUCHED** — the type-scale **value** flip (CSS→TS) stays phase-4 (this restructured the shape + realization, not the value-source). Next: **the paused N+45 doc spike resumes as N+46** (it documented the *fused* axis · stashed/recoverable · resumes against this clean orthogonal shape) · the type-scale value flip stays phase-4. See [`roadmap/N+45.md`](./N+45.md) · [`decisionlog.md` §77 / §54 / §55 / §76](../decisionlog.md).

N+44 · **interactive — single-source the opt-in axis (one SoT, two projections)** — reshape the interactive axis SoT so the web CSS, the web factory gate, and the RN applier all PROJECT from one source (on branch `feat/n44-interactive-single-source` · base `main` @ `72270f4` · [decision 76](../decisionlog.md) · as-built [`N+44.md`](./N+44.md)). **Behaviour-preserving — only the SOURCE moved** (`interactive.css` re-emits byte-identical · the RN snapshots byte-identical · the load-bearing gates). interactive was the one axis still genuinely MULTI-sourced — the opt-in → gate/property/trigger/realization mapping hand-written in THREE readers that could drift. (1) **The SoT reshaped** ([`interactive-effects.ts`](../packages/spec/pipeline/interactive-effects.ts)) from `effects[]` (CSS-rules-as-data) → `opts` (the 3 opt-ins · each `{ trigger, gate, rn, web }` · `rn` pure data: `{prop,from}` node-derived / `{prop,token,shape?}` theme-derived · `web` null where the rule is another axis's — pressColor's `:active` bg-swap is palette's) + `webChrome` (the web-only chrome · affordance/focus/disabledGuard · no RN analog) + `webOrder` (the LOAD-BEARING emit order · pressScale before disabledGuard). (2) **The web CSS emit** ([`interactive-css.js`](../packages/prototype/pipeline/parsers/interactive-css.js)) walks `webOrder` → byte-identical. (3) **The RN applier** ([`resolve.ts`](../packages/rn/factory/resolve.ts)) `flattenPart` + `buildPartRecipe` walk `opts` (the pressColor per-palette-variant `pressedBg` loop PRESERVED) · a compile-time `_OptKeysMatchSchema` pins `keyof opts` to `InteractiveNS`. (4) **The web factory** ([`factory.js`](../packages/prototype/factory/factory.js)) derives the gate attr from `opts[key].gate` via `camelToKebab` (no hardcoded attr string · `INTERACTIVE_GATES` exported). (5) **Guard E** ([`interactive-css.test.js`](../packages/prototype/pipeline/interactive-css.test.js)) pins the factory's gates to the SoT both ways (the one seam the browser-runtime factory can't close by import). **The doc-consumer catch**: @nuri/doc (N+43 · CI-gated byte-identical) consumes `effects` — the brief missed it. Resolved (operator): `effects` survives as a DERIVED projection of `opts`/`webChrome`/`webOrder` (not a second source · proven deep-equal to the original) → @nuri/doc re-emits byte-identical, **untouched** · **tracked for deletion at the doc redesign**. The **dual-stripper constraint** (comma-free `satisfies` · single-line types · no const annotations) documented in the SoT header (both prototype's line-strip + doc's brace-strip load it). Gates green LOCALLY + coordinator-reproduced: **interactive.css byte-identical** · **rn 27/27 + 7 snapshots byte-identical** + **tsc 0** (rn + expo) · **doc 2/2** + `git diff packages/doc/generated/` byte-identical · **spec 46/46** + `git diff packages/spec/build/` byte-identical · **prototype 24/24** (incl. Guard E) · single-source grep clean. Render gate (`demo.html` · preview MCP · `nuri-docs` :8766): factory button carries both reflected gates · affordance + disabled-dim (1→0.4→1) + the pressScale/disabledGuard rules live (order preserved) · console clean · Dark+Lilac re-resolve. **[decision 2](../decisionlog.md) UNTOUCHED** (no §9 flip · only the SoT source unified). Next: **the typography doc reclassification + the uniform axis-doc redesign** (render each axis from its real source · drop the derived `effects`) · **A4c (Foundations docs) renumbered/deferred BEHIND the doc redesign** · A5 playground · A6 codegen. See [`roadmap/N+44.md`](./N+44.md) · [`decisionlog.md` §76 / §73 / §70](../decisionlog.md).

N+43 · **A4b · the axis doc emitter (the Axes nav section)** — add the **axis** doc family to `@nuri/doc`: the 5 namespace-axis reference pages, generated from @nuri/spec's axis SoTs, filling the **Axes** nav section [N+42](./N+42.md) left as a placeholder (on branch `feat/n43-axis-docs` · base `main` @ `eed552e` · [decision 75](../decisionlog.md) · [convergence §5](./convergence.md) · as-built [`N+43.md`](./N+43.md)). **Additive + read-only** — no @nuri/spec change (the axis SoTs are read-only data); the 3 component pages' BODIES stay byte-identical (only their headers re-path). **[decision 2](../decisionlog.md) UNTOUCHED** (the doc-gen generates NO CSS · the [decision 66](../decisionlog.md) generation thesis). The A4 pattern applied to the axes — same shell + nav + helpers, a new emitter; the axes are bespoke ([decision 73](../decisionlog.md) · 2 agnostic + 3 bespoke), so **four shapes, one per SoT**. (1) **The axis-IR builders** ([`axis-ir.js`](../packages/doc/pipeline/axis-ir.js) · NEW · the sibling of `descriptor-ir.js`): **box/stack** ← `resolve-map.ts` (`BOX_FIELDS`/`STACK_FIELDS`) + `property-spelling.ts` → the Field spelling table (input → css/rn name + value-source · the `fill`/expand arm css=`flex`/rn=`—`/per-value cases + a caption · [decision 73](../decisionlog.md) cl.2) · **palette** ← `palette-surface.ts` → the variant XOR chrome role table with RESOLVING swatches (the ONLY axis that resolves tokens) · **interactive** ← `interactive-effects.ts` → the effect set (assembled selector · decls · the automatic/`data-*`-opt-in gate) + the load-bearing ORDER note (data-derived: pressScale before disabledGuard) · **typography** ← `typography-axis.ts` → the `nuri-typography` WRAPPER dispatch (muted + align · the type SCALE scoped OUT → A4c). Plus the `AXIS_DOCS` manifest (source · nav · src · lead · build). (2) **The axis-page rendering** ([`docs.js`](../packages/doc/pipeline/docs.js) · `emitAxisPage` + the 4 kind renderers · reuses the component `swatch`/`ATTR_SEP`/`NO_VALUE` idiom). **`makeRoleResolver`** re-keys the N+22 colour resolver by the L2 role NAME via a var-keyed reverse map over the SAME `token-vars` registry (the role `bg-strong` ≡ the var `--nuri-bg-strong`), NOT a hand re-derivation of spec's convention in doc (the N+42 boundary). (3) **The provenance-header re-path** (the N+42 → A4b carry · DISCHARGED): the GENERATED header extracted into a shared `genHeader(source)`, re-pathed to the @nuri/doc home (`emitter: packages/doc/pipeline/docs.js · re-emit: npm run build -w @nuri/doc`; `source:` now repo-relative). All `.md` regenerated — the 3 component **BODIES byte-identical** (`git diff` = 6/6 = the header re-path only); the 5 axis pages carry it from birth. (4) **`build.js`** split into `buildComponentDocs` + `buildAxisDocs`. (5) **`strip.js`** — a one-token extension (`stripConstAnnotations` now matches BARE `const NAME: T =`, not only `export const` · `resolve-map.ts` annotates its private `ALIGN`/`JUSTIFY`/`FILL` consts that way · doc's depth-aware analogue of prototype's bespoke `resolve-map` strip · the other 4 SoTs unchanged). (6) **The Axes nav landing** ([`axes.md`](../packages/doc/axes.md) · `has_children: true` · the placeholder retired) + **`_config.yml`** (a `generated/axes` → `parent: Axes` DEFAULT · the byte-identical-nav trick). (7) **Guard G extended** ([`docs-drift.test.js`](../packages/doc/pipeline/docs-drift.test.js) · a 2nd test · each `generated/axes/*.md` re-emits ≡ committed + per-page pins + the re-pathed-header pin on both families). Gates green LOCALLY: **doc 2/2** (Guard G · components + axes) + `npm run build -w @nuri/doc` → **8 pages** + the component diff **6/6 header-only** (bodies byte-identical · idempotent re-emit) · **spec 46/46** + `git diff packages/spec/build/` **byte-identical** (the boundary holds) · **prototype 23/23** · **rn 27/27 + 7 snapshots** + **tsc 0** (rn + expo · doc-only session). Render gate (preview MCP · `nuri-doc` :8768 · a throwaway preview mirrors `head_custom.html` · Jekyll can't run locally): all 5 pages render · **console clean** · the palette swatches RESOLVE (`var(--nuri-accent-solid)` → `rgb(18,17,11)` = the printed `#12110b` · `--nuri-bg-strong` → `#f3f1e2`) · the box/stack spelling tables · the interactive effect set + order note · the typography wrapper. **Deferred (LOG-only · accepted)**: the provenance re-path is now DISCHARGED (closed) · A4c (the token/Foundations family · the scales the axes reference by NAME) is the last family · the `llms.txt` rewrite + holistic doc-hygiene stays → phase 5 · `strip.js` is doc's own copy (now richer for `resolve-map`'s bare consts). Next: **A4c** (the token doc emitter · Foundations) · then **A5 `@nuri/playground`** · **A6 codegen surfaces** (+ the factory harness) · **phase 4** (`spec → pure data`) · **phase 5** (the ledger purge). See [`roadmap/N+43.md`](./N+43.md) · [`roadmap/convergence.md`](./convergence.md) · [`decisionlog.md` §75 / §73 / §68](../decisionlog.md).

N+42 · **A4 (slice 1) · stand up `@nuri/doc` (doc owns the MD-gen · convergence phase 2)** — carve the SSG docs package and move the descriptor doc-gen out of `@nuri/spec`, re-sourced onto `spec`'s DATA exports (on branch `feat/n42-doc-site` · base `main` @ `c8e67c8` · [decision 75](../decisionlog.md) · [convergence §5](./convergence.md) · as-built [`N+42.md`](./N+42.md)). **Pure re-plumbing — behaviour-preserving** (the 3 generated pages re-emit BYTE-IDENTICAL · `spec`'s `build/*` byte-identical bar the additive `token-vars.ts` · rn/expo/prototype untouched). **[decision 2](../decisionlog.md) UNTOUCHED** (the doc-gen is READ-ONLY · generates NO CSS · the [decision 66](../decisionlog.md) generation thesis, not §9). The A3 pattern applied to docs — **`spec` = pure data + schema; transforming data → Markdown is `doc`'s job**. (1) **New `@nuri/doc` workspace** (`package.json` · deps `@nuri/spec` + `@nuri/prototype` · the DAG `doc → prototype → spec`) — `npm install` wired it (lockfile +12/−0 · RN tree untouched). (2) **The descriptor doc-gen MOVED + re-sourced**: `parsers/docs.js` → [`packages/doc/pipeline/docs.js`](../packages/doc/pipeline/docs.js) (`emitDocPage`+`buildDocTokenInputs`+`makeColorResolver` · re-sourced from `classifyAll`/`resolveSemanticCrossProduct`/`buildTypeScale` to `@nuri/spec/{tokens, token-vars, palette}` DATA · the size/space/radius re-add the `px` the pipeline strips · the colour resolver reads the var from `token-vars`, the default-scope hex from `tokens`); `docIrFromDescriptor` disentangled from `descriptors.js` → [`descriptor-ir.js`](../packages/doc/pipeline/descriptor-ir.js) (the rest of `descriptors.js` STAYS in spec); [`build.js`](../packages/doc/pipeline/build.js) the orchestrator (the pre-A4 Slice 9). (3) **The node DATA loader** ([`strip.js`](../packages/doc/pipeline/strip.js)) — a brace/angle-DEPTH-aware TS-strip + `data:`-URL import resolves `spec`'s raw-TS exports via the exports map (`import.meta.resolve` · the @nuri/prototype `loadAxisSoT` precedent · node 20 can't import a `.ts` · richer than prototype's line-oriented strip because `build/tokens.ts` has multi-line `type`/inline annotations · empirically validated); the descriptor twins (`.js`) import directly (the recipe precedent). (4) **One minimal NEW `@nuri/spec` DATA export — [`build/token-vars.ts`](../packages/spec/build/token-vars.ts)** (the semantic colour var registry · chrome+accent leaf → CSS-var name · [`semantic.js#emitTokenVarsTs`](../packages/spec/pipeline/parsers/semantic.js) over the same `classifiedGroups` the cascade emit walks · one source two readers · the var names are spec's CSS API, single-sourced as data NOT hand-kebabed in `doc`; dimension scales excluded · render as px literals) + the `./token-vars` export + a slice-5b emit. (5) **`spec` shed the doc concern**: Slice 9 + `DOCS_OUT` + `DOC_COMPONENTS` + the doc imports/re-exports removed; `build/docs/` gone; Slice 7's twin emit STAYS. **Guard G MOVED** → [`packages/doc/pipeline/docs-drift.test.js`](../packages/doc/pipeline/docs-drift.test.js) (re-sourced · the new `doc` CI job gates it); spec's `docs-drift.test.js` shed to **4 guards** (Guard D inlines the IR reshape). (6) **Hand pages ARCHIVED** → `@nuri/doc/archive/` (23 frozen pages · the [N+36](./N+36-legacy-archive.md) `my-vault` pattern · the A4b/A4c regen spec): `spec/pages/{components/*.html (12), foundations/** (9), principles.html, implementation-guide.html}`; `pages/playground/` STAYS (→ A5). **Guard surgery** (the archive forced it): Guard A (pages/components ⊂ llms.txt) RETIRED · Guard C's impl-guide leg RETIRED · Guard D's page-declared-parts cross-check RETIRED (post-flip the descriptor is the SOLE SoT · [§74](../decisionlog.md) · the B1 page/CSS oracle already retired) — Guard D keeps the descriptor-internal `addressed ⊂ anatomy` + the pins. (7) **Jekyll shell + 3-section nav** (`_config.yml`/`_includes`/`index.md` re-homed): **Foundations · Axes · Components** (the cascade order · Components populated by the 3 generated docs · Foundations/Axes placeholders → A4c/A4b). The generated pages nest under "Components" via a Jekyll front-matter DEFAULT (`parent: Components`) — injected at Jekyll time so the generated `.md` stay BYTE-IDENTICAL (doc-gen ⊥ Jekyll nav · the key trick). (8) **`stage.mjs` fixed (stale post-A3)** — rewritten as whole-dir copies of `@nuri/prototype` (factory·primitives·5 namespace CSS·recipes·demo) + `@nuri/spec` (token CSS·descriptor twins·icon registry) + `@nuri/doc` harness, PRESERVING the recipes' cross-package ES-module import graph; `head_custom.html` repointed to the post-flip model (factory + 5 namespace CSS + the 3 recipe MODULES · retired recipe CSS dropped); the generated `.md` no longer staged (Jekyll builds it in place). (9) **CI**: a 5th `gates.yml` job `doc` (Guard G + build + `git diff --exit-code generated/`); `pages.yml` repointed (`website → packages/doc` · `npm run build -w @nuri/doc`). Gates green LOCALLY: **doc 1/1** (Guard G) + `git diff packages/doc/generated/` byte-identical · **spec 46/46** (−1 Guard G moved · −1 Guard A retired) + `git diff packages/spec/build/` byte-identical (only the additive `token-vars.ts`) · **prototype 23/23** byte-identical · **rn 27/27 + 7 snapshots** + **tsc 0** (rn + expo · doc-only carve). Render gate (preview MCP · `nuri-doc` :8768 · `__preview__.html` mirrors `head_custom.html` · Jekyll can't run locally): all 10 custom elements DEFINED via the staged runtime · **console clean** · the factory de-collapsed `<nuri-button>` → `nuri-interactive·stack·box·palette` (solid md bg `rgb(18,17,11)` · 48px · radius 6px) · IconAvatar SVG renders (cross-package `icons.js`) · **theme/accent re-resolve live** (neutral solid light `#12110b` → dark `#fffdf2` · → lilac `#beaaff`) · ≡ the N+41 render gate. **Deferred (LOG-only · accepted · coordinator-confirmed)**: the generated `.md` provenance header is kept VERBATIM (cites the pre-move home) to satisfy the literal byte-identity gate — **TRACKED TO A4b** (NOT a floating follow · the axis emitter regenerates every `.md`, so the re-path folds in FREE there · re-pathing now would dilute this session's load-bearing byte proof · the principled difference from N+41, where the carve already changed the file) · the `llms.txt` consumer manifest still path-lists the archived `spec/pages/*` tree (un-gated after Guard A retired · a holistic post-migration rewrite is deferred doc-hygiene · → phase 5) · `strip.js` is doc's own strip copy (each library owns its loader) · **WATCH the first Pages deploy** — `pages.yml` was broken pre-A4 (the stale `stage.mjs`) and is now repointed but unverified end-to-end (Jekyll can't run locally · the demo runtime IS verified · the nav is config-correct). Next: **A4b** (the axis doc emitter · 5 namespace pages) · **A4c** (the token doc emitter · Foundations) — same shell + nav + `buildDocTokenInputs` helpers · then **A5 `@nuri/playground`** · **A6 codegen surfaces** (+ the factory harness) · **phase 4** (`spec → pure data`) · **phase 5** (the ledger purge · folds in the `llms.txt` rewrite + the provenance re-path). See [`roadmap/N+42.md`](./N+42.md) · [`roadmap/convergence.md`](./convergence.md) · [`decisionlog.md` §75 / §68 / §66 / §48](../decisionlog.md).

N+41 · **A3 · carve `@nuri/prototype` (the post-flip re-cut · convergence phase 2)** — stand up the build-free web library and move the web mechanism out of `packages/spec/lib` (on branch `feat/n41-prototype-carve` · base `main` @ `3e1530a` · [decision 68](../decisionlog.md)'s 6-package split · [convergence](./convergence.md) phase 2 · as-built [`N+41-prototype-carve.md`](./N+41-prototype-carve.md)). **Pure re-plumbing — behaviour-preserving · no decision opened** (dec 2 was already flipped for the namespace layer at [§74](../decisionlog.md) · this only MOVES the mechanism). The **post-flip re-cut** of the held N+37 carve: because [N+38](./N+38-L3c-flip.md) made the namespace CSS GENERATED, it moves to `prototype` **with its generator** (prototype OWNS its emitter · convergence §5 · superseding the pre-flip "CSS stays in spec" plan). (1) **New `@nuri/prototype` workspace** (`package.json` · `README.md` · `dependencies: @nuri/spec` · `devDependencies: postcss`) — `npm install` wired `node_modules/@nuri/prototype` (lockfile +14/−0 · the RN tree untouched). (2) **`git mv` the web mechanism** (history preserved): `factory/` (`factory.js`+`reset.css`) · `primitives/` (11 element `.js` + 7 element `.css`) · `recipes/` (button · icon-avatar · topbar) · `demo/` (`nuri-demo`) · `legacy/` (the frozen pre-axes mechanism). (3) **The namespace-CSS generation moved** — `pipeline/` (`css-preview.js` + the 4 parsers + the 4 freshness tests + the 4 computed-check HTMLs) · the 5 generated outputs → `styles/<ns>.css` (flat). The generator resolves `spec`'s **5 axis SoTs** via `import.meta.resolve('@nuri/spec/<sot>')` (the exports map · the declared data contract) + reads `spec`'s `tokens-semantic.css` (scale vocab) via the spec package root · writes `prototype/styles/` · wired into **`npm run build -w @nuri/prototype`** (its own build). (4) **`@nuri/spec` exports +3** (`./palette-surface` · `./interactive-effects` · `./typography-axis` · TRANSITIONAL pure-data · joining N+39's `./resolve-map`+`./property-spelling`); the shared TS-strip CO-LOCATED in `prototype/pipeline/parsers/strip.js` (spec keeps its own for the token flip); the 2 value-oracle tests + `icon.js` reach spec INTERNALS (`semantic.js` · token CSS · `icons.js`) by cross-package **relative** path (the `icon.js` precedent · build-free · not a declared export). (5) **`spec`'s orchestrator slimmed** ([`tokens-parser.js`](../packages/spec/pipeline/tokens-parser.js) · build-output-NEUTRAL): removed the namespace-CSS slice (`flipNamespaceCss` moved) + the **Slice-4 `@layer tokens` walk** (its `lib/components/*.css` targets moved · fed only the build LOG · the TokenPath union derives from `classifiedGroups` · the resolver stays covered by the synthetic `tokens-parser.test.js`); Slice 8 (`derivePalette`) was already re-sourced to the TS SoTs at N+40, so `spec`'s build no longer reads the namespace CSS → `build/*` byte-identical. (6) **Repoints**: 15 live pages (`../../lib/…` → `../../../prototype/…` · **type-aware**: namespace CSS → `styles/` · element CSS + primitive JS → `primitives/` · recipes → `recipes/` · factory → `factory/` · demo → `demo/`) + the 4 computed-check HTMLs; the recipes import their descriptor twin from `../../spec/build/descriptors/<name>.js` (browser-relative · build-free · no import map). (7) **CI**: a 4th `gates.yml` job `prototype` (test + build + `git diff --exit-code styles/`). **The provenance-header re-path** (coordinator adjustment · the [N+39](./N+39-property-spelling-registry.md) discipline): the 4 emit-header templates re-pathed to the post-carve homes (`prototype/pipeline/css-preview.js` · `npm run build -w @nuri/prototype` · `prototype/factory/factory.js` · `prototype/styles/<ns>.css`; the SoT refs stay `packages/spec/…`-qualified) → the `@layer rules` stay **byte-identical** to pre-carve, ONLY the header lines change. **The build-order invariant** (coordinator adjustment · recorded in [`convergence.md §5`](./convergence.md) + the retro): `npm --workspaces` runs discovery-order (`prototype` before `spec`), NOT topological — benign (prototype reads spec's stable scale KEYS, CI per-job) but a phase-4 token-vocab change needs spec-built-first (the explicit-ordering fix deferred). **DAG**: `prototype → spec` only (`spec` references `prototype` in comments only). Gates green LOCALLY: **prototype 23/23** + `npm run build -w @nuri/prototype` → `git diff packages/prototype/styles/` **header-only** (rules byte-identical) · **spec 48/48** (71 conserved · the 4 namespace tests = 23 subtests moved to prototype) + `git diff packages/spec/build/` **byte-identical** · **rn 27/27 + 7 snapshots** + **tsc 0** (rn + expo · web-only carve · untouched). Render gate ([`demo.html`](../packages/spec/pages/playground/demo.html) · preview MCP · `nuri-docs` :8766): all 9 custom elements defined via the carved library · **console clean** · the factory de-collapsed the button · 4 icon SVGs (cross-package `icons.js`) · **Dark+Lilac re-resolves live** (solid+avatar → lilac `rgb(190,170,255)`) · visually ≡ pre-carve. **Deferred (LOG-only · accepted)**: the factory no-harness gap (→ A6 / a follow) · `scope/README.md` stayed in spec (keeps `implementation-guide.html`'s doc-link valid · `implementation-guide.html`+`principles.html` not repointed · prose doc-links, not live loads) · `strip.js` duplication (each library owns its emitter). Next: **A4 `@nuri/doc`** · **A5 `@nuri/playground`** · **A6 codegen surfaces** (+ the factory harness) · then **phase 4** (`spec → pure data` · the token-vocab flip · where the build-order fix lands) · **phase 5** (the ledger purge). See [`roadmap/N+41-prototype-carve.md`](./N+41-prototype-carve.md) · [`roadmap/convergence.md`](./convergence.md) · [`decisionlog.md` §74 / §68 / §48](../decisionlog.md).

N+40 · **re-source `build/palette.ts` from the TS SoTs** — the second step of `final` ([decision 74](../decisionlog.md)'s "Next" · after [N+39](./N+39-property-spelling-registry.md)) · a small `@nuri/spec`-only, behaviour-preserving re-plumb that **unblocks the A3 carve** (on branch `feat/n40-palette-resource` · base `main` @ `23f0baf` · as-built [`N+40.md`](./N+40.md)). **Pure re-plumbing · no decision opened** (dec 2 untouched · the namespace layer was already flipped at N+38 · §74). Slice 8 (`derivePalette` → `build/palette.ts`, the `@nuri/spec` palette mapping the RN runtime consumes) asserted its frozen `PALETTE_CONTRACT` against the GENERATED `lib/components/{palette,typography}.css` (§74 moved the witness there when the recipe-CSS cross-checks retired); the A3 carve moves that namespace CSS into `@nuri/prototype`, so `spec` reading it would invert the DAG (`spec → prototype`). This re-points the assertion one step UP the cascade — at the **TS SoTs the namespace CSS is generated FROM**: [`pipeline/palette-surface.ts`](../packages/spec/pipeline/palette-surface.ts) (the `surface` role table · bg/fg/pressed) + [`pipeline/typography-axis.ts`](../packages/spec/pipeline/typography-axis.ts) (the `axis` muted dispatch · `var(--nuri-text-muted)`). (1) **`derivePalette` re-sourced** ([`pipeline/parsers/palette.js`](../packages/spec/pipeline/parsers/palette.js)): signature `derivePalette({ surface, typographyAxis }, { classifiedGroups })` · §D reads the muted dispatch rule's `color` decl, §E asserts bg/fg/pressedBg against `surface` modulo three trivial transforms (bare role → `--nuri-<role>` [the new `paintToVar`] · `surface.pressed ↔ pressedBg` · `{literal:'transparent'} ↔ 'transparent'`), honoring the SoT shape (subtle fg-only · chrome no-pressed) + checking BOTH directions (a stray role/channel throws · the [decision 30](../decisionlog.md) reserved cells stay reserved). Dropped `postcss` + `rulesInLayer`/`ruleFor`; kept `varTarget`; the header `Source ·` block re-pathed (baked in on-branch · the N+39 pattern). The cells come from `PALETTE_CONTRACT` (frozen) as before — the CSS reads were always pure ASSERTIONS ([decision 48](../decisionlog.md)), now repointed to the TRUE SoT (strictly better — catches contract-vs-true-SoT drift, not just contract-vs-artifact). (2) **Slice 8 loads the SoTs spec-resident** ([`tokens-parser.js`](../packages/spec/pipeline/tokens-parser.js)): a new local `loadAxisSoT` = `stripTypes` ([`dimension-css.js`](../packages/spec/pipeline/parsers/dimension-css.js) · the shared one-strip impl) + a `data:`-URL import (like `loadDimensions`) — **deliberately NOT** `loadSurface`/`loadAxis`, the namespace-CSS EMITTERS that LEAVE `spec` at the A3 carve (reading the SoT through them would re-invert the DAG); removed the `COMPONENTS_DIR` reads of palette.css/typography.css. (3) **Both `derivePalette` test sites re-sourced** ([`docs-drift.test.js`](../packages/spec/pipeline/docs-drift.test.js) Guard E + G): feed the loaded TS SoTs; Guard E made `async`; the re-emit pin + `EXPECTED_PALETTE` stay green from the new source (resolved TokenPaths · unchanged). **Behaviour-preservation**: `PALETTE_CONTRACT` + the resolve step untouched → the cells are byte-identical BY CONSTRUCTION (the only requirement = the new assertions don't throw · they don't). Gates green LOCALLY: **spec 71/71** · `npm run build -w @nuri/spec` → `git diff packages/spec/build/` = **only the `build/palette.ts` `Source ·` header** (the `export const palette` data byte-identical · `styles/` + `lib/components/` flips byte-identical) · **rn 27/27 + 7 snapshots** (the `@nuri/spec/palette` contract untouched) · **tsc 0** (`@nuri/rn` + `@nuri/expo-demo`) · **decoupling proof**: `derivePalette` reads no `lib/components/*.css` (the only such mentions in `palette.js` are comments). **Deferred (LOG-only · accepted)**: the Slice-4 `@layer tokens` walk still reads `lib/components/*.css` for the build LOG — a SEPARATE, pre-existing coupling A3 owns (the brief scopes this session to "only what Slice 8 reads") · the bespoke axes don't adopt the property-spelling registry here ([decision 73](../decisionlog.md) cl. 2 · the `final` follow · unrelated). Next: **A3 / N+41 — the `@nuri/prototype` carve** (now unblocked — `build/palette.ts` reads the TS SoTs, so the generated palette.css/typography.css can leave `spec` freely) · the package carve's remaining slices · the token residues · the descriptor long-tail. See [`roadmap/N+40.md`](./N+40.md) · [`roadmap/convergence.md`](./convergence.md) · [`decisionlog.md` §74 / §48 / §30 / §34 / §53](../decisionlog.md).

N+39 · **final · the property-spelling registry + `resolve-map.ts` → `@nuri/spec` (RN-free)** — the first step of `final` ([decision 74](../decisionlog.md)'s "Next") · realizes [decision 73](../decisionlog.md) clause 2 (on branch `feat/n39-property-spelling-registry` · base `main` @ `85a8ddd` · as-built [`N+39-property-spelling-registry.md`](./N+39-property-spelling-registry.md)). **Pure re-plumbing — behaviour-preserving · no decision opened** (dec 2 untouched · the namespace layer was already flipped at N+38). Closes two post-flip debts: the live `spec` build was reading its box/stack axis SoT from `@nuri/rn` (the `rn → spec` DAG backwards), and the RN↔CSS property NAME was spelled twice (`resolve-map.ts`'s RN `prop` + `namespace-css.js`'s `LOGICAL_OVERRIDES`/`webProp`). (1) **The registry** ([`pipeline/property-spelling.ts`](../packages/spec/pipeline/property-spelling.ts) · new) — `PROPERTY_SPELLING`: canonical id → `{ rn, css }` for the 16 box/stack name-mappable props. The canonical vocabulary is the CSS-LOGICAL concept camelCased (the de-RN-ification · dec 73 cl.2): `css` = the id kebab-cased (`paddingInline`→`padding-inline`), `rn` carries RN's PHYSICAL de-logicalization (`paddingInline`→`paddingHorizontal` · `inlineSize`→`width`), both stored EXPLICITLY; ids are the registry's own keys (`keyof typeof` · typo-safe); `rn` a plain string (spec stays RN-free). Authored in the strip-trivial `as const satisfies <named type>` style (like `palette-surface.ts`). (2) **`resolve-map.ts` RN-free + relocated** (`git mv packages/rn/factory/ → packages/spec/pipeline/`): `prop: keyof ViewStyle` → `prop: CanonicalId` · dropped `import type { ViewStyle } from 'react-native'` · `StackNS`/`BoxNS` now intra-spec from `./descriptors/schema` (the sub-decision · they always lived in the schema · `contract.ts` only re-exported). The `expand`/`FILL` cases KEPT their multi-prop shape (typed `Record<string, string|number>` · a MECHANISM difference, not a name → NOT a registry entry · dec 73 cl.2). (3) **Both consumers read the registry**: RN ([`resolve.ts`](../packages/rn/factory/resolve.ts) `applyFields`) `set(rnProp(f.prop), …)` where `rnProp = (id)=>PROPERTY_SPELLING[id].rn as keyof ViewStyle` (THE rn boundary cast · imports `@nuri/spec/resolve-map`+`@nuri/spec/property-spelling`); web ([`namespace-css.js`](../packages/spec/pipeline/parsers/namespace-css.js)) `registry[f.prop].css` — **deleted `LOGICAL_OVERRIDES`+`webProp`** (`kebab` KEPT for data-ATTR names only) · new `loadRegistry` (the shared `stripTypes`) · `registry` threaded through `emitNamespaceCss`. (4) **Exports map**: two TRANSITIONAL subpaths `@nuri/spec/resolve-map` + `@nuri/spec/property-spelling` → `pipeline/*.ts` (PURE DATA · safe for the RN bundle · the `rn → spec` DAG · dir-agnostic so the phase-4 re-home changes only the RHS). **Behaviour-preservation**: every canonical→rn reproduces the old `prop`, every canonical→css the old `webProp(prop)` (verified all 16) → `lib/components/{box,stack}/<ns>.css` + `build/` regenerate **byte-identical** · the **RN snapshots do not move** (the dual oracle dec 73 cl.2 named). **DAG fixed**: `css-preview.js` reads `pipeline/resolve-map.ts` (intra-spec) · `resolve.ts` imports `@nuri/spec/resolve-map`. Gates green LOCALLY: **spec 71/71** + `git diff packages/spec/build/` byte-identical + box.css/stack.css RULES byte-identical (only the provenance header re-pathed · 1 line each) · **rn 27/27 + 7 snapshots** byte-identical + **tsc 0** · **expo-demo tsc 0**. **Resolved in review (#67)**: the EMITTED box/stack provenance header (it cited the deleted `packages/rn/factory/resolve-map.ts`) re-pathed to `packages/spec/pipeline/resolve-map.ts` + regenerated — the only box.css/stack.css diff is that 1 header line · every CSS rule byte-identical (the gate restated, not weakened). **Deferred (accepted · LOG-only)**: the other 3 axis SoTs (already in `spec`) adopt the registry in a FOLLOW (not RN-coupled · out of scope) — so `final`'s "home the SoTs into spec" is effectively COMPLETE (only `resolve-map.ts` was ever in rn); `border`/`outline` (dec 30) not pre-added (P11). Next: the registry's adoption by the palette/interactive/typography emits · re-source `build/palette.ts` from `palette-surface.ts` · the package carve. See [`roadmap/N+39-property-spelling-registry.md`](./N+39-property-spelling-registry.md) · [`decisionlog.md` §73 / §74](../decisionlog.md).
