...
The token-SoT inversion is DONE (descriptors B1 · dimensions [§71](../decisionlog.md) · colour [§72](../decisionlog.md)). The **L3 namespace flip** is in progress — box/stack ([N+30](./N+30-L3.1.md) shadow) + palette ([N+33](./N+33-L3b-palette.md) shadow) proven. The axis taxonomy is **2 agnostic (`stack` · `box`) + 3 bespoke (`typography` · `palette` · `interactive`)** ([decision 73](../decisionlog.md) · corrects decision 70's "3 agnostic").

**Finish the L3 axis shadows** (reversible · each proves an emit ≡ the hand CSS):
- **L3b·2 · `interactive`** — the second bespoke axis (the gated `:active`/`:disabled`/`:focus` effects · needs a pseudo-state proof, not the static computed-style of palette).
- **L3.1b · `typography`** — the third bespoke axis (the palette pattern · [decision 73](../decisionlog.md) · its own SoT + emitter). The type SCALE tokens (`--nuri-type-*`) stay CSS-SoT (a later token flip).

**The flip + consolidation:**
- **web-factory harness** — close the no-harness gap BEFORE the factory becomes the sole web renderer (the load-bearing prerequisite).
- **L3c · the flip** (may split) — wire the generated axis CSS live · retire the 3 descriptor recipes (`button`/`icon-avatar`/`topbar`) CSS + JS · the factory = the sole web renderer · repoint the pages · re-source `build/palette.ts` from the TS SoT · extract the **property-spelling registry** ([decision 73](../decisionlog.md) cl. 2) · close the L3.1 Guard-D gap. **[decision 2](../decisionlog.md) fully reverses for the namespace layer** — the de-mess (`lib/` collapses into a coherent web library).
- **final** — home the axis SoTs (`resolve-map.ts` · `palette-surface.ts` · the typography SoT) into `@nuri/spec` (the [decision 68](../decisionlog.md) `rn → spec` DAG).
