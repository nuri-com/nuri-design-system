...
The token-SoT inversion is DONE (descriptors B1 · dimensions [§71](../decisionlog.md) · colour [§72](../decisionlog.md)). The **L3 namespace flip** is in progress — box/stack ([N+30](./N+30-L3.1.md) shadow) + palette ([N+33](./N+33-L3b-palette.md) shadow) + interactive ([N+34](./N+34-L3b-interactive.md) shadow) proven. The axis taxonomy is **2 agnostic (`stack` · `box`) + 3 bespoke (`typography` · `palette` · `interactive`)** ([decision 73](../decisionlog.md) · corrects decision 70's "3 agnostic").
...
- **L3.1b · `typography`** — the third (last) bespoke axis (the palette pattern · [decision 73](../decisionlog.md) · its own SoT + emitter). The type SCALE tokens (`--nuri-type-*`) stay CSS-SoT (a later token flip).
