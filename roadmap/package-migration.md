...
| **A5** ✓ | extract **`@nuri/playground`** ([N+49](./N+49.md)) | `spec/lib/playground` + `spec/pages/playground` → `@nuri/playground` (build-free) · paths rewired (`playground → prototype → spec`) · the dangling A4 `state.js` ref dropped (self-pin) | **S/M** | A3 | DONE · the bench renders build-free · console clean · gates green |
