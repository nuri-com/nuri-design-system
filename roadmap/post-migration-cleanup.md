...
**UPDATE (N+29).** B1 ([decision 69](../decisionlog.md) · merged #54) reversed decision 2 **for the
descriptor layer** (now TS-authored); the consolidated §9 direction is [`docs/cascade.md`](../docs/cascade.md).
The "CSS-is-SoT STANDS · don't correct" caution now applies ONLY to the **token + namespace** layers
(still CSS until their L3 / L2 flip). The descriptor layer is decided; the cascade reframe (the recipe
CSS retires · §9 generates the token-cascade + namespace CSS · **row 6**) is operator-directed.
...
| 6 | **§9 framed as "descriptor → CSS" (per-component / recipe-level) + "inline-CSS-var" web rendering** | `docs/resolver-model.md` §9 · `docs/north-star.md:92,97` · `docs/target-architecture.md:19,297` · `roadmap/{package-migration.md` (B2), `factory-rewrite.md, post-migration-arcs.md}` | **REFRAMED (N+29 · operator-directed · [`docs/cascade.md`](../docs/cascade.md)):** §9-web generates the **token-cascade + namespace CSS** from the TS SoT; the web renders via the **namespace + `data-*`** projection (NOT inline-CSS-vars · NOT per-component); the **recipe CSS retires** (redundant). B1 (decision 69) did the descriptor layer | the **L3** (axes → namespace CSS · retire recipes) + **L2** (token cascade) flip sessions | **flagged** (`resolver-model §9` + `package-migration` Phase-B banners); the rest `table` — correct at the flip |
