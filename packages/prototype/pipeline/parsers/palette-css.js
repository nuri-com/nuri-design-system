...
export function emitPaletteCss(surface) {
  ...
  return [
    ...
    ` *`,
    ` * GENERATED from the SURFACE role table (packages/spec/pipeline/palette-surface.ts)`,
    ...
    ` * retired (git-recoverable), the generator is the sole source, and packages/spec/build/`,
    ` * palette.ts asserts its contract against the same SURFACE SoT (re-sourced at N+40 · the`,
    ` * recipe-CSS cross-checks retired with the recipes · packages/spec/pipeline/parsers/`,
    ` * palette.js). Re-run npm run build -w @nuri/prototype; freshness gated by`,
