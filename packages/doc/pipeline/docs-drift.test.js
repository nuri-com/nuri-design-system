...
const PAGE_CONTRACT = {
  ...
  'icon-button': {
    kind: 'api', source: 'icon-button', title: 'Icon Button', nav: 3,
    cells: [
  ...
  list: {
    kind: 'api', source: 'list', title: 'List', nav: 5,
    cells: [
      "| `accent` | no | `'neutral' | 'lilac' | 'orange'` | theme scope |",
      '| `children` | no | `React.ReactNode` | default content slot |',
    ],
  ...
  'list-action': {
    kind: 'api', source: 'list-action', title: 'List Action', nav: 6,
    cells: [
      '| `onPress` | no | `() => void` | pressable behaviour |',
      '| `disabled` | no | `boolean` | pressable behaviour |',
      '| `accessibilityLabel` | no | `string` | pressable behaviour |',
  ...
  'select-field': {
    kind: 'api', source: 'select-field', title: 'Select Field', nav: 24,
    cells: [
      "| `size` | no | `'md' | 'lg'` | style axis |",
      "| `accent` | no | `'neutral' | 'lilac' | 'orange'` | theme scope |",
      '| `onPress` | no | `() => void` | pressable behaviour |',
  ...
  'tab-bar-item': {
    kind: 'api', source: 'tab-bar-item', title: 'Tab Bar Item', nav: 9,
    cells: [
      "| `accent` | no | `'neutral' | 'lilac' | 'orange'` | theme scope |",
      '| `onPress` | no | `() => void` | pressable behaviour |',
      '| `accessibilityLabel` | no | `string` | pressable behaviour |',
  ...
  topbar: {
    kind: 'api', source: 'topbar', title: 'Topbar', nav: 10,
    cells: [
      "| `surface` | no | `'canvas' | 'transparent'` | component prop |",
  ...
  header: {
    ...
    cells: [
      ...
      '| `onLayout` | no | `(event: LayoutChangeEvent) => void` | RN-only native layout event |',
      '| `ref` | no | `React.Ref<React.ElementRef<typeof RNView>>` | RN-only native host ref |',
      '`Header` owns the painted top safe-area strip.',
    ],
    excludes: ['## Example', '## Anatomy', '## Base', '## Token map'],
  },
  scroll: {
  ...
  'nuri-root': {
    ...
    cells: [
      '`NuriThemeProvider` → `OverlayProvider` → the canvas `View` → `NuriSafeAreaProvider` → `ToastProvider` in contractual order',
      '`NuriThemeProvider`, `OverlayProvider`, `NuriSafeAreaProvider`, and `ToastProvider` remain public for supported piecemeal assembly',
...
const AXIS_CONTRACT = {
  stack: {
    ...
    cells: [
      ...
      '| `fill` | `flex` | — | `grow` → `grow: 1` · `shrink: 0`<br>`grow-shrink` → `grow: 1` · `shrink: 1` · `minInline: 0`<br>`even` → `grow: 1` · `shrink: 1` · `basis: 0` · `minInline: 0`<br>`hug` → `grow: 0` · `shrink: 0` · `crossHug: true` |',
    ],
