...
export function emitInteractiveCss(effects) {
  ...
  return [
    ...
    ` *`,
    ` * GENERATED from the interactive EFFECT set (pipeline/interactive-effects.ts) by`,
    ` * pipeline/css-preview.js — the inverse-spelling of flattenPart`,
    ` * (packages/rn/factory/resolve.ts) — wired into npm run build (pipeline/tokens-`,
    ` * parser.js · the namespace-CSS slice · regenerates IN PLACE). This is the LIVE`,
    ` * interactive namespace CSS: the pages link it and the web factory styles the`,
    ` * nuri-interactive merged node with it. decision 2 reversed for the namespace layer`,
    ` * (decision 74 · executing decision 70 · the L3c flip · N+38) — the hand SoT`,
    ` * retired (git-recoverable), the generator is the sole source. The rule ORDER is`,
    ` * LOAD-BEARING — pressScale before disabledGuard so a disabled control's transform`,
    ` * resolves to none (the equal-specificity pair · the order guard proves it). Re-run`,
    ` * npm run build; freshness gated by pipeline/interactive-css.test.js.`,
    ` * ────────────────────────────────────────────────────────────── */`,
    ``,
    `@layer tokens {`,
