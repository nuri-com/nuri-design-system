...
export function emitNamespaceCss({ ns, title, fields, scaleVocab }) {
  ...
  return [
    `/* ──────────────────────────────────────────────────────────────`,
    ` * NURI · NAMESPACE CSS · ${title} · GENERATED · SHADOW · DO NOT EDIT / DO NOT REPOINT`,
    ` *`,
    ` * The L3.1 reversible spike (decision 70 · docs/cascade.md L3 step 1). GENERATED`,
    ` * from the Field table (packages/rn/factory/resolve-map.ts · ${ns === 'box' ? 'BOX_FIELDS' : 'STACK_FIELDS'})`,
    ` * + the web spelling layer (pipeline/parsers/namespace-css.js) by`,
    ` * pipeline/css-preview.js. SHADOW OUTPUT — proven structurally + computed-`,
    ` * style EQUIVALENT to the hand SoT lib/components/${ns}/${ns}.css (the parity`,
    ` * oracle · pipeline/css-preview.test.js), but NOTHING points at this file:`,
    ` * the live web factory + pages still load the hand CSS. It exists to PROVE`,
    ` * "one table → web CSS" without flipping anything. The L3 flip (a later`,
    ` * session) makes this the source and retires the hand CSS + the recipe layer.`,
