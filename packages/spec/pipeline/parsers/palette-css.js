...
export function emitPaletteCss(surface) {
  ...
  return [
    ...
    ` *`,
    ` * GENERATED from the SURFACE role table (pipeline/palette-surface.ts) by`,
    ` * pipeline/css-preview.js — the inverse-spelling of resolvePalette`,
    ...
    ` * nodes with it. decision 2 reversed for the namespace layer (decision 74 ·`,
    ` * executing decision 70 · the L3c flip · N+38) — the hand SoT retired (git-`,
    ` * recoverable), the generator is the sole source, and build/palette.ts now asserts`,
    ` * its contract against THIS generated CSS (the recipe-CSS cross-checks retired with`,
    ` * the recipes · pipeline/parsers/palette.js). Re-run npm run build; freshness gated`,
    ` * by pipeline/palette-css.test.js.`,
    ` * ────────────────────────────────────────────────────────────── */`,
    ``,
    `@layer tokens {`,
