...
each rule lives on [`packages/spec/pages/principles.html`](./packages/spec/pages/principles.html);
...
| migration | pipeline output · component anatomy/token-mapping · packages/spec/pages/implementation-guide.html | this file |
| operator | README · principles.html · sidebar | this file |

If you find yourself writing a "why" paragraph in this file, it
belongs on `packages/spec/pages/principles.html` (add as a new principle or extend
...
| # | Rule | See |
|---|------|-----|
| 1 | Components consume semantic, never primitive (bare-constant carve-out · see decision 45) | [P3](./packages/spec/pages/principles.html#p3-components-consume-semantic) · [decision 45](./decisionlog.md#45-interaction-family-primitives--cross-component-design-constants---nuri-interaction---n65-post-close-coordinator-polish) |
| 2 | No hex in component code (only in `tokens-primitive.css`) | [P3](./packages/spec/pages/principles.html#p3-components-consume-semantic) |
| 3 | Primitives have `-light` / `-dark` suffix (alpha exempt) | [P2](./packages/spec/pages/principles.html#p2-two-layer-tokens) |
...
| 8 | Mobile-first: `:active` only on components; no `:hover` and no `-hover` suffix | [P6](./packages/spec/pages/principles.html#p6-pressed-only) |
| 9 | Variant naming: `solid` / `soft`; default `soft` | [P7](./packages/spec/pages/principles.html#p7-variant-naming) |
| 10 | DTCG `$type` is inferred via naming prefix | [P9](./packages/spec/pages/principles.html#p9-dtcg-naming) |
| 11 | `rem` for type, `px` for everything else; line-heights unitless | [P8](./packages/spec/pages/principles.html#p8-unit-rule) |
...
| 15 | Token table format is unified — `packages/spec/lib/docs/tokens.js` `renderTable()` emits it natively | [decision 20](./decisionlog.md#20-token-table-format-unified-across-every-page--n2) |
...
| 18 | Component pages serve four readers; anatomy + token-mapping = migration wiring spec; multi-part components REQUIRE machine-readable `data-*` on those tables · **⚠ dec 66: the migration reader is RETIRED (M4) · the four-reader *framing* dies at the `website` doc-gen arc · the `data-*` requirement STAYS (now a descriptor source · Guard D)** | [decision 24](./decisionlog.md#24-component-pages-serve-four-readers-including-migration--n3) · [dec 66](./decisionlog.md) |
...
| 20 | Primitives / components / skills / pages ship only when consumed OR explicitly speculative-reserved with one-line justification | [P11](./packages/spec/pages/principles.html#p11-parsimony) · [decision 30](./decisionlog.md#30-primitive-parsimony--no-speculative-additions--n571) |
| 21 | `packages/spec/build/` is committed and CI drift-guards it (in the `spec` job · rule 22); the `packages/spec/pipeline/docs-drift.test.js` guards (component pages ↔ `llms.txt` · doc-stated counts ↔ live build · descriptor + palette re-emit · the frozen schema shape · Guard F · decision 65 step 5) must stay green (Guard B — `build/components/*` ↔ manifest — retired at Smell-1 · dec 66 with the dir) | [decision 35](./decisionlog.md#35-pipeline-sources-vs-build-outputs-physically-separated--pipeline-source-build-generated-only--n604) · [gates.yml](./.github/workflows/gates.yml) |
| 22 | CI runs **per-workspace** ([`gates.yml`](./.github/workflows/gates.yml): `spec` · `factory` · `expo-demo`). The **`factory` render-smoke gates the contract INTRA-REPO** — it imports `@nuri/spec`'s emitted descriptors+tokens through the `exports` map and RENDERS them on RN (`react-test-renderer`), so a `packages/spec/build/*` change that breaks the RN render fails CI by construction (**R7 closed**). The `button-matrix` migration mirror is **retired** (M4 · [65.11](./decisionlog.md)) — the `factory` render-smoke is the contract gate. | [decision 65.7](./decisionlog.md) / [65.10](./decisionlog.md) / [65.11](./decisionlog.md) · [RISKS R7](./docs/RISKS.md) · [gates.yml](./.github/workflows/gates.yml) |
...
| Skill | Use when |
|---|---|
| [add-component](./skills/add-component.md) | adding a DS component (Tier 1/2/3 surface with an RN equivalent) under `packages/spec/lib/components/<name>/` |
| [add-foundation](./skills/add-foundation.md) | adding a foundation page under `packages/spec/pages/foundations/` (token-driven or sample-driven) |
...
| [add-principle](./skills/add-principle.md) | adding or deprecating a principle on `packages/spec/pages/principles.html` (rare per decision 22) |
...
[`packages/spec/pages/implementation-guide.html`](./packages/spec/pages/implementation-guide.html),
...
- Building a custom table inline when a set of tokens needs displaying —
  use `.nuri-token-table` fed by `packages/spec/lib/docs/tokens.js`
