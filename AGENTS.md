...
| 22 | CI runs **per-workspace** ([`gates.yml`](./.github/workflows/gates.yml): `spec` · `rn` · `expo-demo`). The **`rn` render-smoke gates the contract INTRA-REPO** — it imports `@nuri/spec`'s emitted descriptors+tokens through the `exports` map and RENDERS them on RN (`react-test-renderer`), so a `packages/spec/build/*` change that breaks the RN render fails CI by construction (**R7 closed**). The `button-matrix` migration mirror is **retired** (M4 · [65.11](./decisionlog.md)) — the `rn` render-smoke is the contract gate. | [decision 65.7](./decisionlog.md) / [65.10](./decisionlog.md) / [65.11](./decisionlog.md) · [RISKS R7](./docs/RISKS.md) · [gates.yml](./.github/workflows/gates.yml) |
