...
export const DESCRIPTOR_COMPONENTS = [
  ...
  // retired with the hand recipe (the `<nuri-topbar-content>` placement · the L3c
  // boundary decision 74); the slot-based action bar is now generated by the factory's
  // compound-component capability (a typed sub-component per region · descriptor-driven).
...
// ── Topbar · the CSS parity oracle (deriveTopbar) RETIRED at the topbar-slots
// slice. The hand recipe (the `<nuri-topbar>` host + the `<nuri-topbar-content>`
// pivot CSS the deriver read) is gone — topbar is now a descriptor-ONLY SoT, a
// slot-based action bar generated by the factory's compound-component capability
// (a typed sub-component per leading/center/trailing region · the `even`-flex
// edges for true centring). Like icon-button, it carries no `kind`, so no deriver. ──

const DERIVERS = { button: deriveButton, iconAvatar: deriveIconAvatar };
...
// content row (P11 · the icon-button slice · `prefix 🍎 suffix`); `leading`/`center`/
// `trailing` are the topbar's three regions in left→centre→right row order (the
// topbar-slots slice). The order here fixes the PART_ORDER-sorted anatomy so the web
// twin's row order matches the RN authored order (parity-load-bearing).
const PART_ORDER = ['root', 'leading', 'prefix', 'label', 'icon', 'center', 'suffix', 'trailing', 'content'];
