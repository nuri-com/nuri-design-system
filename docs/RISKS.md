...
**Last updated**: 2026-06-22 (N+28 · **A2.5 · RN dual-tree collapse — NON-VIABLE *and* UNNECESSARY (finding · doc-only)** · the orphaned root `react@19.2.6` / `react-native@0.80.3` dual tree **STAYS**. In-place collapse is impossible on npm 10.8.2 (a plain `npm install` no-ops — it won't rebuild a satisfiable lockfile · `npm dedupe` ERESOLVEs on jest-expo's `react-native@"*"` peer · the M4 "option (b)" `override` is inert in-place); the only mechanism — a from-scratch regen — is non-viable on node 20.19.3 (a **whole-tree refresh** · ~63 transitive deps drift · `metro 0.82→0.83` · `lodash` dropped · breaks jest-expo's preset resolution — the M4 breakage reproduced · the whole-tree drift is the blocker, **not** the node version); **AND it's unnecessary** — a non-RN carve-add (the A3+ `@nuri/prototype`/`doc`/`playground`) is conservative (**+11 / −0** · the RN tree untouched), so the carves just `npm install`. The brief's "`npm install` collapses" premise was **wrong** (inferred from A1's *rename* forcing an ideal-tree rebuild, never a plain install). The jest/Metro single-React/RN workarounds are **RETAINED** (still load-bearing · only their stale "future option" comments repointed). **No risk opened/closed** · no code/lockfile change → gates = the `main` baseline (rn 27/27 + 7 snapshots · spec 26/26 · tsc 0/0 · `build/` byte-identical). See [`roadmap/N+28-A2.5.md`](../roadmap/N+28-A2.5.md).) · Prior 2026-06-22 (N+28 · **A2 · factory-rewrite S4 · web factory generalize** · the runtime web factory ([`lib/runtime/factory.js`](../packages/spec/lib/runtime/factory.js)) now de-collapses **all three** frozen descriptors — S4 extends the SAME engine to icon-avatar/topbar + builds **`nuri-view`** (the static merged-node host · RN `<View>`'s Yoga defaults incl. `flex-shrink:0`) · **ENGINE-ONLY** (the hand recipes STAY · retirement → A6). **A new descriptor-fidelity finding SURFACED** (joins the first-bump agenda · see R1): the **topbar title-type** (lg-em) is NOT in the descriptor (`content` carries no typography) → hand-duplicated in `topbar.js` (bare-string auto-type) + `expo-demo/src/screens/Demo.tsx` (`typeStyle('lgEm')`); **neither factory can generate it** (the RN factory faithfully renders an incomplete descriptor) · the clean fix = a topbar-descriptor design decision (a typed `title` part vs the runtime bare-string convenience) · **DEFERRED**. The icon-avatar **flex-shrink** gap was CLOSED at the primitive level (`nuri-view` `flex-shrink:0` = the RN default · the "explicit fill beats the default" override verified via `@layer` specificity). **NOT §9 · decision 2 STANDS · RN untouched.** Gates: spec 26/26 · rn 27/27 + 7 snapshots · tsc 0/0 · `build/` byte-identical (+ 2 additive `.js` twins · a 1-line header re-emit). No risk opened/closed.) · Prior 2026-06-22 (N+28 · **A1 · `factory → rn` rename** · the first §9-independent package-migration step ([decision 68](../decisionlog.md)). `git mv packages/factory → packages/rn` + every LIVE ref + the CI job `factory:`→`rn:`; the **"factory" engine concept** stays (createNuriComponent · the web factory · `lib/runtime/factory.js`). **This file's R1/R3/R5 verification-role pointers repointed** to [`@nuri/rn`](../packages/rn/) (L683/827/988 only) — the dated changelog + Closed-risk entries below keep the M2/N+19-era `@nuri/factory` name (tense-frozen records · renaming would falsely date `@nuri/rn` to M2). **Pure rename · no behaviour change**; the lockfile is a mechanical `factory`→`rn` string rename that **preserves the dual-version RN tree** (`npm install` would collapse it · "don't-force-it" · [N+19-M4](../roadmap/N+19-M4.md)). **No risk opened/closed.** Gates: rn 27/27 + 7 snapshots byte-identical · spec 26/26 · expo-demo tsc 0 · `build/` byte-identical. **Operator action at merge**: branch-protection required check → **remove `factory`, add `rn`** (net `spec`+`rn`+`expo-demo` · M3 decision B).) · Prior 2026-06-22 (N+27 · **factory-rewrite S3 · the web factory** · the browser `buildComponent` ([`lib/runtime/factory.js`](../packages/spec/lib/runtime/factory.js)) de-collapses Button into `<nuri-pressable><nuri-typography>` styled from the descriptor via `data-*` [**option A** · reuse the hand `@layer` CSS · no `resolve-map.ts`], proven equivalent to `<nuri-button>` [**14/14** computed-style cells + pixel-parity · values trace to `build/docs/button.md`]. **The R1.5 "no default-per-axis" finding RE-SURFACES in the runtime mirror** [the factory falls back to the FIRST value · `variant`→`solid` · while `<nuri-button>` defaults to `soft` · compared at explicit axis values · **still OPEN** · no contract change]. A hand-written [`lib/runtime/reset.css`](../packages/spec/lib/runtime/reset.css) carries the native-`<button>` host normalization, kept OUT of the §9-generation-target namespace CSS [operator-directed · the §9 boundary]. **NOT §9 · decision 2 STANDS.** Gates: spec 26/26 · factory 27/27 + 7 snapshots · tsc 0/0.) · Prior 2026-06-21 (N+23 · **website doc-gen · arc #1 increment 2** · the emitter generalized Button → all three `DESCRIPTOR_COMPONENTS` and the token-map gained the resolved value beside each token path. **Two of the four R1/R1.5 first-bump findings now SURFACE in the generated docs** [faithful · still OPEN]: the **stringly-typed boolean axis** [topbar `center` · `false`·`true` · `center=false`→no token-map rows] + the **`subtle` §11** fg-only variant [icon-avatar]. **NOT §9** [read-only on the descriptor · CSS/schema untouched]; no risk opened or closed. Gates: spec 26/26 · factory 27/27 + 7 snapshots · tsc 0/0.) · Prior 2026-06-20 (N+21 · **Smell-1** · the decision-45 interaction baseline relocated to its own transversal emit `build/interaction.ts`, `build/components/` retired — the **R1 "no transversal interaction emit" finding is CLOSED** [Closed risks below]; the factory seam reads `@nuri/spec/interaction`, Guard B retired, docs de-staled. Gates: spec 25/25 · factory 27/27 + 7 snapshots · tsc 0/0 · `build/` = 8 deletions + `interaction.ts`. A **Smell-1.1** dead-code tail [the vestigial per-component walk + 3 emitted-file header echoes] is deferred.) · Prior 2026-06-20 (N+19 · M4 · **`button-matrix` retired — the migration is COMPLETE**: the type-only
...
- ✓ **N+28·A2 update — the topbar title-type is a DESCRIPTOR-FIDELITY gap** (surfaced, NOT
  fixed · joins the first-bump agenda). The web factory's S4 generalization (the runtime mirror
  de-collapsing topbar from the frozen descriptor · [`N+28-A2.md`](../roadmap/N+28-A2.md)) made
  concrete that the lg-em title typography is **not in the SoT**: the descriptor's `content` part
  carries no `typography` namespace ([decision 64](../decisionlog.md) made it "PURE LAYOUT"), so the
  lg-em is **hand-duplicated in two places** — [`topbar.js`](../packages/spec/lib/components/topbar/topbar.js)
  (the web recipe's bare-string `.nuri-type-lg--em` auto-type) AND
  [`packages/expo-demo/src/screens/Demo.tsx:31`](../packages/expo-demo/src/screens/Demo.tsx) (the RN
  consumer's `TopbarTitle` → `typeStyle('lgEm')`). **Neither factory can generate it** — the RN
  factory is NOT buggy, it faithfully renders an incomplete descriptor (the render-smoke passes the
  title as an explicit `<Text>`; the web factory renders a typed title identically — verified 22px/600
  — only the recipe's bare-string *auto-typing* differs). The clean fix is a **topbar-descriptor
  DESIGN decision** — a typed `title` part vs the bare-string-conditional convenience (runtime
  child-inspection, not static descriptor data) — **DEFERRED to the first-versioned-bump agenda**
  alongside the no-default-per-axis (R1.5) · stringly-typed boolean axes · `subtle` fg-only findings.
  (Surfaced, not fixed · the schema is frozen · Guard F · the S3/R1.5 precedent.)
...
verification role is now the [`@nuri/rn`](../packages/rn/)
...
[`@nuri/rn`](../packages/rn/) render-smoke — it gates
...
[`@nuri/rn`](../packages/rn/) `react-test-renderer` render-smoke mounts
...
**Scope**. Closes ONE of the four R1/R1.5 consumability findings; the other three (no default-per-axis · stringly-typed boolean axes · `subtle` §11 doc-reconciliation) stay open as the Digital-cash first-bump agenda. **(N+23 update · the website doc-gen now makes two of them VISIBLE in the generated docs** — the stringly-typed boolean axis [topbar `center` · `false`·`true` · `center=false`→no token-map rows] + `subtle` [icon-avatar's fg-only variant] — **faithful, still open** · no contract change taken · the [`N+23.md`](../roadmap/N+23.md) retire-gate is gated on the default-per-axis + axes⊂props gaps.) A **Smell-1.1** dead-code tail (the now-vestigial per-component `@layer` walk + the 3 emitted-file header echoes still naming `build/components/<name>.ts`) is deferred — see [`roadmap/N+21.md`](../roadmap/N+21.md).
