...
* **The coherence rule (component-api.test.js Channel 2 · replaces "target is an interactive view"):** three directions, each a NAMED failure — every `behaviour.pressable.target` is an `el:'pressable'` part; every `el:'pressable'` part is THE declared target (the schema carries at most one `behaviour.pressable`, so an untargeted pressable is an unreachable press host); `interactive` flags live ONLY on `el:'pressable'` parts (verified to hold descriptor-wide before encoding — no static part carries an effects opt-in). Mutation-checked: flipping one root back to `'view'` without touching its behaviour fails on direction 1.
