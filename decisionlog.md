...
### 65.12 amendment · PR #130 (2026-07-02) · `PaletteVariant` gains `outline` — the palette's first BORDER channel

**The 1st deliberate post-freeze `PaletteVariant` bump (decision 65 step 5 · "post-freeze changes are versioned").** `PaletteVariant` = `'solid' | 'soft' | 'ghost' | 'subtle' | 'outline'`; the Guard-F pin (`scripts/docs-drift.test.js` `FROZEN_SCHEMA.leafUnions.PaletteVariant`) moved with it, annotated in the pin's precedent style (`even` · `minWidth` · `aspectRatio`). This DISCHARGES decision 30's "`palette` outline = RESERVED · mapped-not-built" reservation (held since N+5.7.1 · restated by B2b/B3/the property-spelling registry): the reservation is now BUILT. (`box` maxWidth · `onSolid.muted` · the `interactive` value-override stay reserved.)

* **The row (palette-surface.ts · the surface table's first `border` channel):** `outline = { bg: transparent (literal) · fg: text-muted · border: border-subtle }` — an optional-by-shape fourth channel beside bg/fg/pressed. No pressed swap (a static-surface role, like the chrome slot); NO `fgMuted` cell — its fg IS text-muted, so the muted swap would substitute text-muted for text-muted (absent-by-omission · the subtle/solid precedent · the RN resolver's fg fallback covers a muted outline node · settled in the PR-#130 review pass).
* **Realization (bespoke on both targets · NOT the property-spelling registry):** RN — `resolvePalette` adds `borderColor` + `borderWidth: PALETTE_BORDER_WIDTH` (= 1 · asserted ≡ the web `--nuri-border-1` by resolve.test.ts); web — the palette emit writes the `border: var(--nuri-border-1) solid var(--nuri-border-subtle)` shorthand. The border-WIDTH stays a passthrough token — its promotion into the dimension SoT is deliberately DEFERRED to the catalog-arc gap batch (it lands with the Separator primitive, which wants the same token; `property-spelling.ts` states this actual state).
* **First consumer:** icon-avatar's `outline` variant (its fifth variant-axis row · descriptor → codegen adapters → RN resolver + web CSS emit → doc pages → the playground bench). Web↔RN parity verified at computed-style level (48×48 border-box · 1px `#dddac9` · fg `#666455` on both targets).
* **Known parity wrinkle (documented, not resolved):** RN `<Text variant="outline">` paints border GEOMETRY (`node.view` spreads into the text style · primitives/Text.tsx) while web `nuri-typography` ignores `variant` entirely — the first palette value whose twin divergence is geometric. The realization decision (reflect palette attrs on the web element vs narrow Text's RN surface) belongs to the catalog-arc gap batch.
...
* **Why structure, not flag:** "which JSX host does this part render as" is a static, per-descriptor STRUCTURAL fact — but it was smeared across three places and derived differently per projection: the anatomy said `el:'view'`, the RN renderer picked `<Pressable>` off the BEHAVIOUR channel at runtime (`ctx.behaviour.pressable?.target === node.name` · an `if` inside `case 'view'`), and the web factory picked `<nuri-pressable>` off the INTERACTIVE flags (`ns.interactive ? … : …`). Both projections already materialized the distinction in their OUTPUT; the primitive layer already exported `Pressable` first-class. The descriptor schema was the ONLY layer where pressable was a derivation instead of structure. Fixed at the source: the renderers are now a PURE switch over `el` (philosophy "the renderer just renders"), the behaviour channel carries only what it is — runtime handlers (`onPress`/`disabled`/`accessibilityLabel`) — and the `interactive` flags choose only EFFECTS (unchanged · decision 65.4).
* **The three flips (the whole catalog's pressable set):** button · icon-button · tab-bar-item root anatomies — exactly the descriptors that declare `behaviour.pressable`, all targeting `root`. Every other part stays `'view'|'text'|'icon'`.
* **The coherence rule (component-api.test.js Channel 2 · replaces "target is an interactive view"):** FOUR directions, each a NAMED failure — every `behaviour.pressable.target` is an `el:'pressable'` part; every `el:'pressable'` part is THE declared target (the schema carries at most one `behaviour.pressable`, so an untargeted pressable is an unreachable press host); `interactive` flags live ONLY on `el:'pressable'` parts (verified to hold descriptor-wide before encoding — no static part carries an effects opt-in); and every `el:'pressable'` part DECLARES `interactive` in base or a variant value (review §9 · "onPress must not exist independent of interactivity" — the pre-65.13 rule's scan, restored as direction 4 in the PR-#132 review pass so the three legs are a true equivalence, not a one-way implication). Mutation-checked at two layers: flipping one root back to `'view'` without touching its behaviour fails on direction 1 (spec source); stripping `interactive` from a pressable root in the committed TWIN fails on direction 4 (the guard reads the twins — a source-only mutation is the re-emit gate's catch, not Channel 2's).
* **Render output BYTE-IDENTICAL on both targets (the hard constraint):** RN jest snapshots pass WITHOUT `-u`; the web factory suite + parity bench unchanged; `packages/prototype/styles/` byte-identical (no CSS change). The legitimate DATA diffs: the three descriptor twins + `packages/rn/generated/data/recipes.ts` — the baked recipe mirrors anatomy `el` verbatim (`{ el }` per part · scripts/parsers/recipes.js), so anatomy-mirroring data moves WITH the anatomy (the kickoff brief expected recipes unchanged; the tree says recipes carry `el` — understood and accepted as the twins' class of diff). Adapters · palette · tokens · icons · interaction: UNCHANGED.

**Base**: decision 65 step 5 (the enforced freeze · versioned bumps) · 65.6 (Guard F · the pin that moved) · 65.3/65.4 ("behaviour ≠ data" — sharpened, not reversed: HANDLERS stay runtime behaviour; the HOST is structure) · decision 67 (the el→primitive lock — `<nuri-pressable>` was already the el-host, now `el` names it). **Realizes**: `packages/spec/components/{schema.ts · button.ts · icon-button.ts · tab-bar-item.ts}` → `packages/rn/runtime/renderer.tsx` (the pure switch · `case 'pressable'` · the leaf null-check narrowed to text/icon) · `packages/prototype/factory/factory.js` (host pick keys on `el`) · `scripts/parsers/components-api.js` (root host = view|pressable) · the regenerated twins + `recipes.ts` · the Guard-F pin move · the Channel-2 coherence rule · the §0.1 rewrite.

### 65.14 amendment · 2026-07-03 · `PaletteChrome` gains `'transparent'` for docked screen chrome

**The 1st deliberate post-freeze `PaletteChrome` bump (decision 65 step 5 · "post-freeze changes are versioned").** `PaletteChrome` = `'canvas' | 'subtle' | 'strong' | 'transparent'`; the Guard-F pin (`scripts/docs-drift.test.js` `FROZEN_SCHEMA.leafUnions.PaletteChrome`) moved with it. This is a palette-row contract bump, not page CSS: web emits `.nuri-palette[data-chrome="transparent"] { background: transparent; color: var(--nuri-text-primary) }`, and RN emits the matching `theme.chrome.transparent` role.

* **First consumer:** `TabBar` gains the public axis `surface: 'canvas' | 'transparent'`, defaulting to `canvas` to preserve the existing bottom-bar render. The transparent value is for product screens where a screen-local `Dock` overlays bottom chrome above a scroll body; the TabBar stays a dumb open container and does not learn route state, safe area, fixed positioning, or viewport ownership.
* **Why chrome, not special CSS:** transparent is a complete palette pair (transparent bg + normal foreground), so it belongs in the palette surface table beside the other chrome roles. This keeps the web/RN authoring shape and resolution path identical: descriptor data → generated adapter → palette resolver, with no playground-only styling escape hatch.

**Base**: decision 65 step 5 (the enforced freeze · versioned bumps) · 65.6 (Guard F · the pin that moved) · decision 67/70 (bespoke-but-single-sourced palette axis) · decision 49 (TabBar has no built-in top divider). **Realizes**: `packages/spec/{components/schema.ts · axes/palette-surface.ts · components/tab-bar.ts}` → regenerated palette/style/component/doc outputs + RN render/type coverage for `TabBar surface="transparent"` inside a screen-local Dock.
...
## 78. §9 — decision 2 reversed for the type COMPOSITE (executing decision 70 · the value flip §77 deferred) · N+52

**Operator-ratified · the type-scale value flip [§77](../decisionlog.md) explicitly deferred to phase 4 ("the type-scale **value** flip (CSS→TS) stays phase-4").** Records the **state transition of [decision 2](../decisionlog.md)** (CSS is the SoT) for the type COMPOSITE, exactly as [§71](../decisionlog.md) (dimensions) / [§72](../decisionlog.md) (colour) did for the other token layers: the six `--nuri-type-*` step composites are now WRITTEN INTO `styles/tokens-primitive.css` from a TS SoT, not read out of it — and the ledger says so rather than misstate "CSS is SoT." This flips the type COMPOSITE; it does **NOT** complete the token-SoT inversion — the font-size/weight PRIMITIVES (+ border / font families / durations) stay hand-CSS (a Phase 4·3 residue · see Scope).

- **The SoT is `pipeline/typography.ts` — a self-contained, readable INLINE table of text styles, deliberately NOT the dimension `{ref}` shape.** Each step is its own complete style in px — `md: { fontSize: 17, lineHeight: 1.29, letterSpacing: -0.02, weight: 'regular' }` — the value of the table is seeing fontSize+lineHeight+letterSpacing+weight together in one place; with only six steps the indirection into the font primitives does not pay. The emit (`parsers/type-css.js` · `flipTypeCss` · Slice 0) is an in-place postcss rewrite of ONLY the `--nuri-type-*` declarations (everything else verbatim), DE-REFERENCED end to end: `--nuri-type-{step}-size` `var(--nuri-font-size-N)` → the rem literal (px÷16 · `17` → `1.0625rem` · the current spelling), `--nuri-type-{step}-weight` `var(--nuri-font-weight-X)` → the weight literal (`regular` → `400` · resolved from the primitive). So the `--nuri-font-size-*` / `--nuri-font-weight-*` PRIMITIVES go back to being a foundational hand-CSS layer (`shell.css`'s concern) — **the fake type↔shell coupling dissolves, it is not documented as a half-flip.**
- **Behaviour-preserving — computed-equivalent, NOT byte-identical** (the [§77](../decisionlog.md)/N+45 gate posture, not §71's byte-identity). `build/tokens.ts` `type` (the RN reader · re-sourced: `buildTypeScale` reads the inline table straight, resolving only the weight name against the primitive) + the RN snapshots (29/29 + 7) stay **byte-identical** (the resolved values don't change). The ONLY changed bytes are the `--nuri-type-*` block of `styles/tokens-primitive.css` (var→inline · resolved value identical) — incl. one cosmetic normalization, `xl` `line-height: 1.20` → `1.2` (the SoT authors the number `1.2` · value-identical). Everything else in `build/*` + `styles/*` + `@nuri/prototype` styles + `@nuri/doc` generated is byte-identical. `styles/typography.css` (the web reader · `[data-type-style]`) is untouched.
- **Scope: the type COMPOSITE only — the font-size/weight PRIMITIVES stay hand-CSS** (the [§77](../decisionlog.md) open question, resolved by grounding). `--nuri-font-size-*` / `--nuri-font-weight-*` are a FOUNDATIONAL layer shared far beyond type (`shell.css` consumes them at 40+ sites; `typography.css`'s emphasis rule references `--nuri-font-weight-semibold`), so flipping them into a "typography" SoT would be a misnomer. De-referencing left `--nuri-font-size-30` / `--nuri-font-size-57` (the `xl`/`3xl` steps · the only ones `shell.css` does NOT use) with no consumer → **RESERVED** in the guardrail (P11 · the full-ramp pattern · their membership is re-decided at the font-primitive residue flip). `--nuri-font-weight-regular` stays consumed by `shell.css` (verified · not orphaned · NOT deleted). The `2xl` gap stays reserved. EMPHASIS stays orthogonal ([§77](../decisionlog.md)) — the emitter resolves `--nuri-font-weight-semibold` directly; no per-step token.
- **The drift guard is the N+31 dimension-cascade harness pattern.** `tokens-parser.test.js`'s type guard is re-pointed at the SoT: an INDEPENDENT restated-scale oracle (the px sizes · lineHeights · trackings · weights · NOT read from the SoT or the CSS) is resolved BOTH through the SoT (`buildTypeScale`) AND through the live (now inline) CSS values; plus the in-place de-referenced re-emit byte-identity + the on-disk `tokens.ts` re-emit. Non-tautological — proven by mutation (perturb a SoT `fontSize` / `lineHeight` → the oracle bites).

**Base**: [decision 2](../decisionlog.md) (CSS SoT · **now reversed for the type composite** · STILL STANDS for the residual hand-CSS primitives — the font-size/weight families · `--nuri-border-*` · font families · the duration/interaction families — until their own residue flips · Phase 4·3) · [29](../decisionlog.md) (line-height is a unitless ratio · no absolute primitive) · [54](../decisionlog.md)/[55](../decisionlog.md) (the type-step · the directly-accessed `type` namespace) · [77](../decisionlog.md) (the de-fusion · this executes the value flip it deferred · its computed-equivalent gate posture) · [35](../decisionlog.md) (pipeline source vs build generated · the `styles/` generated region is the §71 in-place hybrid trade) · [48](../decisionlog.md) (one source, two readers) · [70](../decisionlog.md) (the cascade · this executes a token residue) · [71](../decisionlog.md)/[72](../decisionlog.md) (the dimension/colour flips · the state-transition pattern + the de-risked mechanism this mirrors). **Realizes**: `packages/spec/pipeline/{typography.ts [the TS SoT · inline text-styles table · new] · parsers/type-css.js [the in-place de-referenced emit · new] · parsers/type.js [re-sourced onto the SoT] · tokens-parser.js [Slice 0 + the re-sourced buildTypeScale call] · tokens-parser.test.js [the re-pointed drift guard + the 30/57 reservations]}` + the regenerated `--nuri-type-*` block in `styles/tokens-primitive.css` · as-built [`roadmap/N+52.md`](../roadmap/N+52.md). **Next** (Phase 4·3): the font-size/weight primitive flip (which re-decides the 30/57 ramp membership) + the remaining small token residues (`--nuri-border-*` · font families) → TS · then the codegen move so `@nuri/spec` is no-deps/no-scripts (drop postcss · [decision 68](../decisionlog.md) the package carve). With type, the token VOCABULARIES are TS-authored (dimensions · colour · type); the shared primitives + organizational remain.

## 79. The token codegen relocated to root `scripts/` — `@nuri/spec` reaches no-deps / no-scripts · N+53

**Operator-ratified · the structural EXIT-path step of [decision 68](../decisionlog.md)'s Phase 4 ("`spec` → pure data") · NOT a [decision 2](../decisionlog.md) flip.** The `postcss`-using token codegen — the orchestrator + the 12 emitters + the 6 drift-guard tests — **moved out of `@nuri/spec` into a root `scripts/` directory**, so the spec package ends with **no `devDependencies` and no `build`/`test` scripts**: pure data its consumers import (the `.ts` token SoTs + `descriptors/*.ts` + `lib/`/`styles/`/`pages/` + the frozen `build/*` contract · [Guard F](../packages/spec/build/tokens.ts)), regenerated by an external tool. **Behaviour-preserving — the generator's LOCATION moved, its OUTPUT did not** (`build/*` + `styles/*` + `lib/components/icon/icons.js` re-emit **byte-identical** from the new home).

- **The mechanical trick is a one-line re-anchor.** Every path constant in the codegen is `REPO_ROOT`-relative, and the codegen reads the `.ts` SoTs via path constants (data-URL `stripTypes` loads · `loadDimensions`/`loadColours`/`loadTypography`/…), NOT via `import … from '*.ts'` — VERIFIED zero such imports against the tree. So `REPO_ROOT = resolve(__dirname, '..')` → `resolve(__dirname, '../packages/spec')` (the new home is one level shallower than the old `pipeline/`) keeps **every** `resolve(REPO_ROOT, 'styles/…')` etc. resolving to the same files. The `.ts` SoTs STAYED in `pipeline/` (the `./resolve-map`, `./dimensions`, … subpath exports keep resolving · no consumer-path churn) — only the codegen `.js` moved. The `git mv` of `tokens-parser.js` + `parsers/*` (preserving the `parsers/` subdir) left the inter-emitter relative imports untouched (pure renames).
- **The codegen is root TOOLING, NOT a 7th package** — the [decision 68](../decisionlog.md) 6-package count stays intact (`spec`/`rn`/`prototype`/`doc`/`playground` + the `expo-demo` example · root `scripts/` is build tooling, not a workspace). This settles `package-architecture.md §7`'s open codegen-home **against** a `@nuri/codegen` package ([convergence §5](../roadmap/convergence.md)'s "no codegen package").
- **The `@nuri/prototype` → `scripts/parsers/semantic.js` Guard-C coupling.** Two prototype namespace-CSS freshness tests ([`palette-css.test.js`](../packages/prototype/pipeline/palette-css.test.js) · [`typography-css.test.js`](../packages/prototype/pipeline/typography-css.test.js)) reach across the package boundary into the colour-cascade resolver (`readSemanticRules`/`buildPrimitiveMap`/`resolveSemanticCrossProduct`) as Guard C's muted-token value oracle — the "build-free relative reach · NOT a declared export" pattern. That resolver is `semantic.js`, which **moved with the codegen**, so the two imports were repointed `../../spec/pipeline/parsers/semantic.js` → `../../../scripts/parsers/semantic.js` (the only `@nuri/prototype` touch · test-import-only, no build-logic change — `css-preview.js` reads only its own `./parsers/*` + spec's exported SoTs + the generated `tokens-semantic.css`, all unaffected). **Recorded so a future codegen move knows the dependency exists.** (The anti-goal "don't touch prototype's codegen" meant don't rework `css-preview.js`/its `postcss` — a forced relative-import repoint is not that.)
- **The root-build-order fix landed** (the deferred N+41 footgun). Root `build` is now `node scripts/tokens-parser.js && npm run build --workspaces --if-present` — the codegen runs FIRST (it writes `spec`'s `styles/tokens-semantic.css`), THEN the workspaces (`prototype`'s `css-preview.js` reads that generated CSS cross-package · DAG `prototype → spec`). The N+41 "fix when it bites" build-order invariant is discharged. Root `test` is `node --test scripts/*.test.js && npm test --workspaces --if-present`; `"type": "module"` added to the root `package.json` (it now owns ESM codegen). `postcss` moved from `spec`'s devDep to the root devDep (lockfile **5/5 lines** · pure declaration relocation · already hoisted · the RN dual-tree untouched · no metro/jest-expo/lodash churn).
- **CI: the `spec` job re-pointed, the required-check NAME preserved** (the [N+28](../roadmap/N+28-A2.5.md) A1 `factory`→`rn` precedent · avoid the branch-protection rename dance). The job now runs `node --test scripts/*.test.js` (the relocated drift guards · folded into the existing job — one token-pipeline gate, minimal churn) + `node scripts/tokens-parser.js` + `git diff --exit-code packages/spec/build/ packages/spec/styles/` (the `styles/` byte-identical diff ADDED — the codegen regenerates it in place). The other 4 jobs (`prototype`/`rn`/`expo-demo`/`doc`) are unchanged + self-sufficient against the committed tree.

**Base**: [decision 68](../decisionlog.md) (the 6-package carve · Phase 4 "`spec` → pure data" · the codegen is tooling not a package) · [decision 35](../decisionlog.md) (pipeline source vs build generated · `build/` + `styles/` committed · the re-emit gate) · [decision 2](../decisionlog.md) (**UNTOUCHED** — this is ORGANIZATIONAL · no SoT moved, no CSS authored/generated · the residual hand-CSS primitives stay hand-CSS) · [convergence §5](../roadmap/convergence.md) (the no-`@nuri/codegen` generation model · the build-order invariant this discharges). **Realizes**: `scripts/{tokens-parser.js · parsers/* · the 6 *.test.js}` (moved · `REPO_ROOT` re-anchored) · `package.json` (root · `type:module` + `postcss` devDep + the codegen-first build + the `scripts/*.test.js` test) · [`packages/spec/package.json`](../packages/spec/package.json) (no `scripts`, no `devDependencies`) · the 2 repointed `@nuri/prototype` tests · [`.github/workflows/gates.yml`](../.github/workflows/gates.yml) (the `spec` job · name preserved) · as-built [`roadmap/N+53.md`](../roadmap/N+53.md). **Next** (Phase 4·3 residues · optional polish): the font-size/weight primitives + `--nuri-border-*` + font families + durations → TS (orthogonal to this · the hand-CSS primitives are already pure data). Then Phase 5 (the ledger purge + `MEMORY.md`) → EXIT.

## 80. The projection model — `@nuri/spec` is data, projections carry resolution · N+54

**Operator-ratified · docs + ledger only · no code.** The concrete architecture after the [N+52](../roadmap/N+52.md)/[N+53](../roadmap/N+53.md) inversion: **`@nuri/spec` is PURE DATA** (`tokens`/`axes`/`components`/`icons` · no codegen, no deps, no generated artifacts, no logic); every consumer is a **projection** that resolves spec's data and **owns that resolution**. Full model: [`docs/projection-model.md`](../docs/projection-model.md).

- **Two projections.** **web** (CSS → `@nuri/prototype` · refs → `var()` cascade · the **browser** resolves at runtime) · **RN** (the resolved-semantic layer → `@nuri/rn` · refs flattened to **hex at build** · primitives never shipped · **no TS→CSS→TS round-trip** · runtime only **selects** the theme/accent slice).
- **Colour is the only context-variant axis** (theme ⊥ accent · **accent-major** refs into Radix-style ramps `{light,dark}` · chrome theme-only). The build resolves everything resolvable; runtime selects only the context-variant (colour today · `density`/`fontScale` latent). The RN provider is therefore **colour-only · opt-in** (a static consumer pays nothing · Unistyles is a perf detail of `@nuri/rn`, not a requirement).
- **What dies**: `build/` in `spec` · the TS→CSS→TS round-trip (`build/tokens.ts` walked from CSS) · the `build/descriptors/*.ts` verbatim copies · the fossil dirs (`pipeline/`, `build/`).

**Base**: [decision 79](../decisionlog.md) (codegen → root `scripts/`; this completes "`spec` → pure data" by removing the *generated* too) · [decision 70](../decisionlog.md) (one source, two projections — this is its concrete, gated form) · [decision 63](../decisionlog.md) (the accent×theme self-scope cascade — now expressed as the accent-major flat\|`{light,dark}` shape) · [decision 2](../decisionlog.md) (places generated OUTPUT outside `spec` — the resolved contract now *leaves* `spec` entirely). **Supersedes** [decision 35](../decisionlog.md) **on one point only** — `build/` committed *in `spec`* as the resolved contract: the resolved contract now lives in the projection packages, never in `spec`. §35's substance **survives** — the source/generated split and the "generated is never hand-edited · re-emit ≡ committed" gate discipline still hold; they just apply in the projection package that owns the output. And the model docs [`resolver-model.md`](../docs/resolver-model.md) / [`emit-model.md`](../docs/emit-model.md) / [`composition-model.md`](../docs/composition-model.md) (their decisions live in [§64](../decisionlog.md)/[§65](../decisionlog.md)/[§70](../decisionlog.md) · to be collapsed in the Phase-5 doc purge). **Realizes** (the slice plan · [`docs/projection-model.md`](../docs/projection-model.md) §8): accent reshape (role-major → accent-major + the RN selection provider) → add the new accents → `spec` data-only (kill `build/` · drop the descriptor `.ts` copies · move the resolver to `rn`) → the catalog (Phase 6).

## 81. Dimension leaves are flat; `border.1` is the shared hairline SoT · N+55

**Operator-ratified · DS foundation slice for the Nuri home screen.** The dimension SoT no longer carries the intermediate `px` scale. `space`, `size`, `radius`, `ratio`, and the new minimal `border` table own their terminal values directly: pixel dimensions emit `Npx` (with `0` unitless), ratios emit bare numbers, and `radius.full` remains the `9999px` sentinel. The old generated `--nuri-px-*` family is retired; generated CSS now emits the semantic dimension leaves directly plus the single shipped hairline `--nuri-border-1: 1px`.

- **`border.1` is promoted into `packages/spec/tokens/dimensions.ts` as the only border-width leaf.** No speculative `border.0` / `border.2` / `border.4`; focus-ring width remains hardcoded and outside this slice. Web palette outlines and web Separator keep using `--nuri-border-1`, but that variable is now generated from the dimension SoT.
- **RN consumes the same hairline token.** `@nuri/rn/generated/data/tokens.ts` exports `border`, `PALETTE_BORDER_WIDTH` reads `border[1]`, and outline palette borders no longer carry a resolver-local literal.
- **RN `Separator` becomes a contracted parity primitive.** It is horizontal-only, defaults `ySpace="sm"`, accepts the local `none | xs | sm | md | lg | xl` rhythm without widening global `SpaceLeaf`, paints `border[1]`, and uses the scoped theme's `border.subtle` colour. The primitive is exported through the RN barrels and pinned by primitive parity coverage.
